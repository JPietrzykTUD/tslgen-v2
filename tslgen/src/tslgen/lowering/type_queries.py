...
from tslgen.lowering.selected_specializations import (
    selected_specialization_binding_diagnostics,
    selected_specialization_extension_name,
...
def build_selected_type_environment(
    ...
    diagnostics: list[Diagnostic] = list(
        selected_specialization_binding_diagnostics(context)
    )
...
def _lower_type_expression_value(
    ...
    *,
    ...
    if _ALIAS_NAME_RE.fullmatch(expression) is not None:
        ...
        selected_binding = selected_specialization_type_value(
            context,
            expression,
            source,
        )
