...
def test_m214_renders_direct_cpp_intrinsic_call() -> None:
    invocation = _direct_invocation("intrin<_mm_add_epi32>(left, right)")

    result = render_cpp_intrinsic_invocation_call(invocation)

    assert result.diagnostics == ()
    assert isinstance(result.call, CppRenderedIntrinsicCall)
    assert result.call.invocation is invocation
    assert result.call.call_text == "_mm_add_epi32(left, right)"
    assert result.call.immediates == ()
    assert result.call.source == invocation.source


def test_m214_renders_composed_cpp_intrinsic_call() -> None:
    ...
    assert result.diagnostics == ()
    assert isinstance(result.call, CppRenderedIntrinsicCall)
    assert result.call.call_text == "_mm256_add_epi32(left, right)"
    assert result.call.immediates == ()


def test_m214_renders_empty_argument_payload_as_empty_call() -> None:
    ...
    assert result.call.call_text == "_mm_lfence()"


def test_m214_preserves_opaque_nested_tsil_looking_argument_payload() -> None:
    ...
    assert result.call is not None
    assert (
        result.call.call_text
        == "_mm_blend_epi32(left, intrin_compose<nested, suffix=si32>(right))"
    )


def test_m214_preserves_immediate_metadata_without_rewriting_call_text() -> None:
    invocation = _composed_invocation(
    ...
    assert result.diagnostics == ()
    assert isinstance(result.call, CppRenderedIntrinsicCall)
    assert result.call.call_text == "extract(data, 4)"
...
def test_m214_diagnoses_non_cpp_invocation_values() -> None:
    invocation = _direct_invocation("intrin<svadd_s32_x>(pg, left, right)", backend="rust")
...
def test_m214_diagnoses_unsupported_invocation_shapes() -> None:
    result = render_cpp_intrinsic_invocation_call(object())
...
def test_m214_public_cpp_backend_imports_are_available() -> None:
    ...
    assert CppIntrinsicCallText("_mm_add_epi32(left, right)") == (
        "_mm_add_epi32(left, right)"
    )
    assert CppIntrinsicCallRenderResult.__name__ == "CppIntrinsicCallRenderResult"
    assert CppRenderedIntrinsicCall.__name__ == "CppRenderedIntrinsicCall"
    assert callable(render_cpp_intrinsic_invocation_call)
