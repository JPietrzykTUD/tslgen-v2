...
EXPECTED_RULES = (
    ConcreteIntegerGenerationTypeRule("si8", "si8", "ui8", True),
    ConcreteIntegerGenerationTypeRule("ui8", "si8", "ui8", False),
    ConcreteIntegerGenerationTypeRule("si16", "si16", "ui16", True),
    ConcreteIntegerGenerationTypeRule("ui16", "si16", "ui16", False),
    ConcreteIntegerGenerationTypeRule("si32", "si32", "ui32", True),
    ConcreteIntegerGenerationTypeRule("ui32", "si32", "ui32", False),
    ConcreteIntegerGenerationTypeRule("si64", "si64", "ui64", True),
    ConcreteIntegerGenerationTypeRule("ui64", "si64", "ui64", False),
)


def source_span(line: int = 1) -> SourceSpan:
    return SourceSpan(SourceLocation(Path("rule-source-fixture.tsl"), line, 3))
...
class ConcreteIntegerGenerationRuleSourceTests(unittest.TestCase):
    def test_builds_accepted_rule_set_from_tsldata_type_groups(self) -> None:
        ...
        self.assertEqual(rule_set.rules, EXPECTED_RULES)
        self.assertEqual(
    ...
    def test_rule_ordering_is_deterministic_from_unsorted_type_groups(self) -> None:
        ...
        second = build_concrete_integer_generation_rule_set(
            accepted_singleton_groups()
        )

        self.assertTrue(first.is_ok, first.diagnostics)
        self.assertTrue(second.is_ok, second.diagnostics)
        self.assertEqual(first.unwrap().rules, EXPECTED_RULES)
        self.assertEqual(first.unwrap(), second.unwrap())
    ...
    def test_reports_missing_singleton_tags(self) -> None:
        ...
        result = build_concrete_integer_generation_rule_set(groups)
        ...
        self.assertTrue(
            any("si64" in diagnostic.message for diagnostic in result.diagnostics)
        )

    def test_reports_missing_companion_pairs(self) -> None:
        result = build_concrete_integer_generation_rule_set(
            accepted_singleton_groups(),
        ...
        assert_diagnostic(
            ...
            code="TSL-DOMAIN-GEN-RULE-COMPANION-MISSING",
            severity="error",
        )
        self.assertIn("si32", result.diagnostics[0].message)
        self.assertIn("ui32", result.diagnostics[0].message)

    def test_reports_inconsistent_singleton_rule_data(self) -> None:
        groups = tuple(
            type_group("si32", ("ui32",), line=7)
            if group.name == "si32"
            else group
            for group in accepted_singleton_groups()
        )
        ...
        diagnostic = next(
            ...
            if diagnostic.code == "TSL-DOMAIN-GEN-RULE-SINGLETON-INCONSISTENT"
        )
        assert_diagnostic(
            self,
            diagnostic,
            code="TSL-DOMAIN-GEN-RULE-SINGLETON-INCONSISTENT",
            severity="error",
            path="rule-source-fixture.tsl",
            line=7,
            column=3,
        )
    ...
    def test_rejects_wildcard_and_group_selectors_as_selected_tags(self) -> None:
        groups = accepted_singleton_groups() + (
            ...
            type_group("?i64", ("si64", "ui64")),
            type_group("si?", ("si8", "si16", "si32", "si64")),
            type_group("ui?", ("ui8", "ui16", "ui32", "ui64")),
            type_group("idqword", ("si32", "ui32", "si64", "ui64")),
        )

        for type_tag in ("?i?", "?i64", "si?", "ui?", "idqword"):
            with self.subTest(type_tag=type_tag):
                ...
                assert_diagnostic(
                    self,
                    result.diagnostics[0],
                    code="TSL-DOMAIN-GEN-RULE-TAG-UNSUPPORTED",
                    severity="error",
                )
                self.assertIn(type_tag, result.diagnostics[0].message)

    def test_rejects_unsupported_non_integer_and_concrete_looking_tags(self) -> None:
        for type_tag in ("f32", "f64", "ptr", "mask", "imask", "si128"):
            with self.subTest(type_tag=type_tag):
                result = build_concrete_integer_generation_rule_set(
    ...
    def test_rejects_unknown_selected_tags(self) -> None:
        ...
        assert_diagnostic(
            self,
            result.diagnostics[0],
            code="TSL-DOMAIN-GEN-RULE-TAG-UNKNOWN",
            severity="error",
        )
    ...
    def test_concrete_looking_unselected_catalog_tags_do_not_become_rules(
        ...
        result = build_concrete_integer_generation_rule_set(
            accepted_singleton_groups() + (type_group("si128"),),
        )
