...
import tslgen.lowering._array_body_backend_handoff as lowering_array_body_backend_handoff
import tslgen.lowering._array_body_completion_package as lowering_array_body_completion_package
...
class LoweringBoundaryTests(unittest.TestCase):
    ...
    def exact_array_body_structural_package(
        self,
        *,
        selected_type_tag: str = "si16",
    ) -> ExactArrayBodyStructuralPackageIr:
        return_emission = self.exact_return_emission_structural_request(
            selected_type_tag=selected_type_tag,
        )
    ...
    def exact_array_lowering_completion_package(
        self,
        *,
        selected_type_tag: str = "si16",
    ) -> ExactArrayLoweringCompletionPackageIr:
        inventory = self.exact_array_backend_deferred_request_inventory(
            selected_type_tag=selected_type_tag,
        )
        result = lower_exact_array_lowering_completion_package(inventory)
        if not result.is_ok:
            raise AssertionError(result.diagnostics)
        return result.unwrap()

    def exact_array_backend_handoff_request(
        self,
        *,
        selected_type_tag: str = "si16",
    ) -> ExactArrayBackendHandoffRequestIr:
        completion = self.exact_array_lowering_completion_package(
            selected_type_tag=selected_type_tag,
        )
        result = lower_exact_array_backend_handoff_request(completion)
        if not result.is_ok:
            raise AssertionError(result.diagnostics)
        return result.unwrap()
    ...
    def test_lower_candidates_assembles_exact_array_body_envelopes_from_typed_skeletons(
        ...
        for baseline_impl, implementation in zip(
            ...
            with self.subTest(candidate_id=implementation.candidate_id):
                ...
                self.assertIs(structural_package.source_sequence, structural_sequence)
                ...
                self.assertEqual(
                    len(implementation.array_lowering_completion_packages),
                    1,
                )
                self.assertIs(
                ...
                self.assertEqual(
                    len(implementation.lowering_completion_gap_inventories),
                    1,
                )
                self.assertIs(
                    implementation.generation_stages[-1].output,
    ...
    def test_exact_array_initialization_stage_pipeline_lowers_selected_paths(
        ...
        for selected_type_tag, token_text in cases:
            with self.subTest(selected_type_tag=selected_type_tag):
                ...
                structural_package = pipeline.array_body_structural_packages[0]
                self.assertIs(structural_package.source_envelope, array_envelope)
                self.assertIs(
                    structural_package.helper_set_completion,
                    helper_set_completion,
                )
                self.assertIs(structural_package.declaration_shell, declaration_shell)
                self.assertIs(structural_package.source_sequence, structural_sequence)
                self.assertIs(structural_package.predicate_path, predicate_path)
                self.assertIs(
                    structural_package.post_branch_call_site,
                    post_branch_call_site,
                )
                self.assertIs(
                    structural_package.return_emission,
                ...
                self.assertIs(backend_inventory.source_package, structural_package)
                self.assertEqual(len(pipeline.array_lowering_completion_packages), 1)
                completion_package = pipeline.array_lowering_completion_packages[0]
                self.assertIs(completion_package.source_package, structural_package)
                self.assertIs(completion_package.source_inventory, backend_inventory)
                self.assertEqual(len(pipeline.array_backend_handoff_requests), 1)
                backend_handoff = pipeline.array_backend_handoff_requests[0]
                self.assertIs(
                    backend_handoff.source_completion_package,
                    completion_package,
                )
                self.assertEqual(len(pipeline.operation_packages), 1)
                operation_package = pipeline.operation_packages[0]
                assert operation_package.exact_array_backend_handoff is not None
                self.assertIs(
                    operation_package.exact_array_backend_handoff.source_request,
                    backend_handoff,
                )
                self.assertEqual(
    ...
    def test_exact_array_initialization_stage_pipeline_lowers_no_body_paths(
        ...
        for selected_type_tag in ("si8", "ui8"):
            with self.subTest(selected_type_tag=selected_type_tag):
                ...
                self.assertEqual(len(pipeline.array_backend_handoff_requests), 1)
                self.assertEqual(len(pipeline.operation_packages), 1)
                self.assertEqual(len(pipeline.stages), 17)
    ...
    def test_exact_array_initialization_stage_pipeline_no_skeleton_is_empty(
        ...
        self.assertEqual(pipeline.array_lowering_completion_packages, ())
    ...
    def test_exact_array_initialization_stage_pipeline_matches_lower_candidates_tail(
        ...
        self.assertEqual(
            implementation.array_lowering_completion_packages,
            pipeline_result.array_lowering_completion_packages,
        )
        self.assertEqual(
            implementation.array_backend_handoff_requests,
    ...
    def test_m77_exact_array_pipeline_snapshot_records_stage_facts(self) -> None:
        ...
        self.assertEqual(
            snapshot.steps[-4].depends_on,
            ("array_body_structural_package",),
        )
        self.assertEqual(
            snapshot.steps[-5].depends_on,
            (
                "array_body_envelope",
                "array_initialization_helper_set_completion",
                "array_initialization_declaration_shell",
    ...
    def test_m83_stage_contract_accepts_all_accepted_outputs(self) -> None:
        ...
        operation_package = lower_lowering_operation_package(
            statement,
            candidate_id="candidate-1",
            source_location=SourceLocation(Path("operation.tsl"), 7, 11),
        ).unwrap()
        manifest = (
            lowering_completion_manifest.lower_stage8_lowering_completion_manifest(
                operation_package,
            ).unwrap()
        )
        ...
        cases = (
            ...
            (
                "array_lowering_completion_package",
                self.exact_array_lowering_completion_package(),
            ),
            (
                "array_backend_handoff_request",
                self.exact_array_backend_handoff_request(),
            ),
            ("lowering_operation_package", operation_package),
            ("lowering_completion_manifest", manifest),
        )
    ...
    def test_m86_mini_tsil_pipeline_stage_identity_and_determinism(self) -> None:
        ...
        self.assertIs(
            ...
            first_impl.operation_packages[0],
        )
        self.assertIs(
            first_impl.generation_stages[2].output,
            first_impl.lowering_completion_manifests[0],
        )
        self.assertIs(
    ...
    def test_m87_exact_array_body_pipeline_snapshot_preserves_return_identity(
        ...
        self.assertIs(result.stages[-6].output, return_emission)
        snapshot = result.pipeline_snapshot
        self.assertEqual(
            tuple(step.stage_name for step in snapshot.steps[-7:-5]),
            (
                "post_branch_intrinsic_call_site_structural_request_lowering",
                "return_emission_structural_request_lowering",
            ),
        )
        self.assertIs(snapshot.steps[-6].stage, result.stages[-6])
        self.assertEqual(
            snapshot.steps[-6].produced_fact.kind,
            "return_emission_structural_request",
        )
        self.assertIs(snapshot.steps[-6].produced_fact.value, return_emission)
        self.assertEqual(snapshot.steps[-6].produced_fact.key, return_emission.key)
        self.assertEqual(
            snapshot.steps[-6].depends_on,
            (
                "array_body_structural_sequence",
                "post_branch_intrinsic_call_site_structural_request",
            ),
    ...
    def test_m88_exact_array_body_structural_package_lowers_m87_sources(self) -> None:
        return_emission = self.exact_return_emission_structural_request(
            selected_type_tag="si16",
        )

        class M87OnlyLoweredImplementationSource:
            def __init__(
                self,
                request: ExactReturnEmissionStructuralRequestIr,
            ) -> None:
                self.return_emission_structural_requests = (request,)

        sources = (
            return_emission,
            GenerationLoweringStage(
                stage="return_emission_structural_request_lowering",
                output=return_emission,
            ),
            LoweredImplementation(
            ...
            M87OnlyLoweredImplementationSource(return_emission),
        )

        for source in sources:
            with self.subTest(source=type(source).__name__):
                result = lower_exact_array_body_structural_package(
                    source,
                    GenerationContext(
                        selected_candidate_id=return_emission.candidate_id,
                ...
                self.assertIs(
                    ...
                    return_emission.source_post_branch_call_site,
                )
                self.assertIs(
                    package.predicate_path,
                    return_emission.source_post_branch_call_site.source_predicate_path,
                )
                self.assertIs(package.source_sequence, return_emission.source_sequence)
                self.assertIs(
                    package.declaration_shell,
                    return_emission.source_sequence.declaration_shell,
                )
                self.assertIs(
                ...
                self.assertIs(
                    package.source_envelope,
                    return_emission.source_sequence.source_envelope,
                )
                self.assertEqual(
                    tuple(member.kind for member in package.members),
                    (
                        "declaration_shell",
                        "predicate_path",
                        "selected_update",
                ...
                self.assertEqual(
                    tuple(member.role_ordinal for member in package.members),
                    (0, 1, 2, 3, 4),
                )
                self.assertIs(package.members[0].fact, package.declaration_shell)
                self.assertIs(package.members[1].fact, package.predicate_path)
                self.assertIs(package.members[2].fact, package.predicate_path)
                self.assertIs(package.members[3].fact, package.post_branch_call_site)
                self.assertIs(package.members[4].fact, return_emission)
                self.assertFalse(hasattr(package, "body_semantics"))
                self.assertFalse(hasattr(package, "backend_translation"))
                self.assertFalse(hasattr(package, "renderer_value"))

    def test_m88_structural_package_reports_source_and_context_diagnostics(
        self,
    ) -> None:
        return_emission = self.exact_return_emission_structural_request()
        duplicate = LoweredImplementation(
            candidate_id=return_emission.candidate_id,
            status="lowered",
            return_emission_structural_requests=(return_emission, return_emission),
        )
        missing = LoweredImplementation(
            candidate_id=return_emission.candidate_id,
            status="lowered",
        ...
        class MalformedM87OnlyLoweredImplementationSource:
            def __init__(self) -> None:
                self.return_emission_structural_requests: tuple[object, ...] = (
                    object(),
                )
        ...
        cases = (
            (
                ...
                GenerationLoweringStage(
                    stage="post_branch_intrinsic_call_site_structural_request_lowering",
                    output=return_emission.source_post_branch_call_site,
                ),
            ...
            (
                "malformed_entry",
                MalformedM87OnlyLoweredImplementationSource(),
                "TSL-LOWER-ARRAY-BODY-PACKAGE-SOURCE-UNSUPPORTED",
                "ExactReturnEmissionStructuralRequestIr",
            ),
            (
        ...
        for name, source, code, message in cases:
            with self.subTest(name=name):
                result = lower_exact_array_body_structural_package(source)
        ...
        assert_diagnostic(
            self,
            result.diagnostics[0],
            code="TSL-LOWER-ARRAY-BODY-PACKAGE-CONTEXT-MISMATCH",
            severity="error",
            path="tsldata/primitives/load_store/array.tsl",
            line=111,
        )

    def test_m88_structural_package_reports_member_and_provenance_diagnostics(
        self,
    ) -> None:
        missing_helper = self.exact_return_emission_structural_request()
        object.__setattr__(
            missing_helper.source_sequence.declaration_shell,
            "source_helper_set_completion",
            None,
        )
        out_of_order = self.exact_return_emission_structural_request()
        roles = out_of_order.source_sequence.roles
        object.__setattr__(
            out_of_order.source_sequence,
            "roles",
            (roles[1], roles[0], roles[2], roles[3], roles[4]),
        )
        provenance_mismatch = self.exact_return_emission_structural_request()
        other_predicate_path = self.exact_predicate_path_structural_request(
            selected_type_tag="si32",
        )
        object.__setattr__(
            provenance_mismatch.source_post_branch_call_site,
            "source_predicate_path",
            other_predicate_path,
        )
        cases = (
            (
                "missing_helper",
    ...
    def test_m88_structural_package_stage_follows_return_emission(self) -> None:
        selection = self.selection_for("lower_generation_size_byte_branch_chain")
        item, envelope = self.size_byte_branch_chain_item_and_envelope("si32")
        skeleton = self.exact_array_body_skeleton_for_envelope(envelope)
        ...
        implementations_with_package = tuple(
            lowered.candidate_id
            for lowered in plan.implementations
            if lowered.array_body_structural_packages
        )
        self.assertEqual(implementations_with_package, (item.candidate_id,))

    def test_m88_exact_array_body_pipeline_snapshot_preserves_package_identity(
        self,
    ) -> None:
        pipeline = self.exact_array_initialization_stage_pipeline("si32")
        ...
        self.assertIs(result.stages[-5].output, package)
        snapshot = result.pipeline_snapshot
        self.assertEqual(
            snapshot.steps[-5].stage_name,
            "array_body_structural_package_assembly",
        )
        self.assertIs(snapshot.steps[-5].stage, result.stages[-5])
        self.assertEqual(
            snapshot.steps[-5].produced_fact.kind,
            "array_body_structural_package",
        )
        self.assertIs(snapshot.steps[-5].produced_fact.value, package)
        self.assertEqual(snapshot.steps[-5].produced_fact.key, package.key)
        self.assertEqual(
            snapshot.steps[-5].depends_on,
            (
                "array_body_envelope",
    ...
    def test_m88_structural_package_is_deterministic_for_reordered_inputs(
        self,
    ) -> None:
        selection = self.selection_for("lower_generation_size_byte_branch_chain")
        baseline = lower_candidates(selection)
        ...
        def package_keys(plan) -> dict[str, tuple[tuple[object, ...], ...]]:
            return {
                implementation.candidate_id: tuple(
                    package.key
                    for package in implementation.array_body_structural_packages
                )
                for implementation in plan.unwrap().implementations
            }
    ...
    def test_m88_structural_package_module_import_boundary(self) -> None:
        ...
        imported_forbidden: list[str] = []
        tree = ast.parse(inspect.getsource(lowering_array_body_package))
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                for alias in node.names:
        ...
        self.assertEqual(imported_forbidden, [])
        self.assertIs(
            lowering_boundary.lower_exact_array_body_structural_package,
            lowering_array_body_package.lower_exact_array_body_structural_package,
        )
        self.assertIs(
            lower_exact_array_body_structural_package,
            lowering_array_body_package.lower_exact_array_body_structural_package,
    ...
    def test_m89_exact_array_body_pipeline_snapshot_preserves_inventory_identity(
        ...
        self.assertIs(inventory.source_package, package)
        self.assertIs(result.stages[-4].output, inventory)
        snapshot = result.pipeline_snapshot
        self.assertEqual(
            snapshot.steps[-4].stage_name,
            "array_backend_deferred_request_inventory",
        )
        self.assertIs(snapshot.steps[-4].stage, result.stages[-4])
        self.assertEqual(
            snapshot.steps[-4].produced_fact.kind,
            "array_backend_deferred_request_inventory",
        )
        self.assertIs(snapshot.steps[-4].produced_fact.value, inventory)
        self.assertEqual(snapshot.steps[-4].produced_fact.key, inventory.key)
        self.assertEqual(
            snapshot.steps[-4].depends_on,
            ("array_body_structural_package",),
        )
    ...
    def test_m90_lowering_completion_package_lowers_m89_sources(self) -> None:
        inventory = self.exact_array_backend_deferred_request_inventory(
            selected_type_tag="si16",
        )
        package = inventory.source_package

        class M88M89LoweredImplementationSource:
            def __init__(
                self,
                accepted_package: ExactArrayBodyStructuralPackageIr,
                accepted_inventory: ExactArrayBackendDeferredRequestInventoryIr,
            ) -> None:
                self.array_body_structural_packages = (accepted_package,)
                self.array_backend_deferred_request_inventories = (
                    accepted_inventory,
                )
        ...
        sources = (
            ...
            LoweredImplementation(
                ...
                status="lowered",
                array_body_structural_packages=(package,),
                array_backend_deferred_request_inventories=(inventory,),
            ),
            M88M89LoweredImplementationSource(package, inventory),
        )

        for source in sources:
            with self.subTest(source=type(source).__name__):
                result = lower_exact_array_lowering_completion_package(
                    source,
                    GenerationContext(
                        selected_candidate_id=inventory.candidate_id,
                ...
                self.assertTrue(result.is_ok, result.diagnostics)
                completion = result.unwrap()
                dependency = completion.value_backend_uninit_array_dependency
                member = inventory.value_backend_uninit_array
                self.assertIsInstance(
                    completion,
                    ExactArrayLoweringCompletionPackageIr,
                ...
                self.assertIs(completion.source_package, package)
                self.assertIs(completion.source_inventory, inventory)
                self.assertIs(
                    completion.source_declaration_shell,
                    package.declaration_shell,
                )
                self.assertIs(
                    completion.source_helper_set_completion,
                    package.helper_set_completion,
                )
                self.assertEqual(completion.package_members, package.members)
                self.assertEqual(completion.unresolved_dependencies, (dependency,))
                self.assertEqual(dependency.kind, "value_backend_uninit_array")
                self.assertEqual(dependency.request_kind, "backend_value")
                self.assertEqual(dependency.policy, "deferred_backend_value")
                self.assertIs(dependency.source_inventory_member, member)
                self.assertIs(
                    dependency.source_deferred_backend_uninit,
                    member.source_deferred_backend_uninit,
                )
                self.assertIs(
                    dependency.source_request_record,
                    member.source_request_record,
                )
                self.assertIs(
                    dependency.source_deferred_backend_uninit,
                    package.helper_set_completion.unresolved_backend_uninit,
                ...
                self.assertEqual(completion.source_location, inventory.source_location)
                self.assertFalse(hasattr(completion, "backend_translation"))
                self.assertFalse(hasattr(completion, "renderer_value"))
                self.assertFalse(hasattr(dependency, "resolved_backend_value"))
                self.assertFalse(hasattr(dependency, "backend_map_entry"))

    def test_m90_lowering_completion_package_reports_source_diagnostics(
        self,
    ) -> None:
        inventory = self.exact_array_backend_deferred_request_inventory()
        package = inventory.source_package
        other_inventory = self.exact_array_backend_deferred_request_inventory(
            selected_type_tag="si32",
        )
        ...
        class RuntimeSource:
            def __init__(
                ...
                inventories: object = (inventory,),
            ) -> None:
                self.array_body_structural_packages = packages
                self.array_backend_deferred_request_inventories = inventories

        class MissingInventorySource:
            def __init__(self) -> None:
                self.array_body_structural_packages = (package,)

        class MissingPackageSource:
            def __init__(self) -> None:
                self.array_backend_deferred_request_inventories = (inventory,)
        ...
        cases = (
            ...
            (
                ...
                object(),
                "TSL-LOWER-ARRAY-COMPLETION-SOURCE-UNSUPPORTED",
                "M89 backend-deferred inventory",
            ),
            (
                "missing_package",
                RuntimeSource(packages=(), inventories=(inventory,)),
                "TSL-LOWER-ARRAY-COMPLETION-PACKAGE-MISSING",
                "array_body_structural_packages",
            ),
            (
                "missing_inventory",
                RuntimeSource(packages=(package,), inventories=()),
                "TSL-LOWER-ARRAY-COMPLETION-INVENTORY-MISSING",
                "array_backend_deferred_request_inventories",
            ),
            (
                "missing_inventory_attr",
                MissingInventorySource(),
                "TSL-LOWER-ARRAY-COMPLETION-INVENTORY-MISSING",
                "M89",
            ),
            (
                "missing_package_attr",
            ...
            (
                "duplicate_package",
                RuntimeSource(packages=(package, package)),
                "TSL-LOWER-ARRAY-COMPLETION-PACKAGE-MULTIPLE",
                "exactly one",
            ),
            (
                "duplicate_inventory",
                RuntimeSource(inventories=(inventory, inventory)),
                "TSL-LOWER-ARRAY-COMPLETION-INVENTORY-MULTIPLE",
                "exactly one",
            ),
            (
                "malformed_package",
                RuntimeSource(packages=(object(),)),
                "TSL-LOWER-ARRAY-COMPLETION-SOURCE-UNSUPPORTED",
                "ExactArrayBodyStructuralPackageIr",
            ),
            (
                "malformed_inventory",
                RuntimeSource(inventories=(object(),)),
                "TSL-LOWER-ARRAY-COMPLETION-SOURCE-UNSUPPORTED",
                "ExactArrayBackendDeferredRequestInventoryIr",
            ),
            (
                "non_tuple_package",
                RuntimeSource(packages=[package]),
                "TSL-LOWER-ARRAY-COMPLETION-SOURCE-UNSUPPORTED",
                "tuple",
            ),
            (
                "non_tuple_inventory",
                RuntimeSource(inventories=[inventory]),
                "TSL-LOWER-ARRAY-COMPLETION-SOURCE-UNSUPPORTED",
                "tuple",
            ),
            (
                "package_inventory_mismatch",
                RuntimeSource(packages=(package,), inventories=(other_inventory,)),
                "TSL-LOWER-ARRAY-COMPLETION-PACKAGE-INVENTORY-MISMATCH",
                "same accepted M88 package",
            ),
        )

        for name, source, code, message in cases:
            with self.subTest(name=name):
                result = lower_exact_array_lowering_completion_package(source)
        ...
        assert_diagnostic(
            self,
            result.diagnostics[0],
            code="TSL-LOWER-ARRAY-COMPLETION-CONTEXT-MISMATCH",
            severity="error",
            path="tsldata/primitives/load_store/array.tsl",
            line=105,
        )

    def test_m90_lowering_completion_package_reports_boundary_diagnostics(
        self,
    ) -> None:
        def diagnostic_codes_for(
        ...
        cases: list[
            tuple[str, ExactArrayBackendDeferredRequestInventoryIr, str]
        ] = []

        source_location_mismatch = self.exact_array_backend_deferred_request_inventory()
        object.__setattr__(
            source_location_mismatch,
            "source_location",
            SourceLocation(Path("tsldata/primitives/load_store/array.tsl"), 106, 15),
        )
        cases.append(
            (
                "source_location_mismatch",
        ...
        context_mismatch = self.exact_array_backend_deferred_request_inventory()
        object.__setattr__(context_mismatch, "target_extension", "neon")
        cases.append(
            (
                "context_mismatch",
                context_mismatch,
        ...
        member_set_mismatch = self.exact_array_backend_deferred_request_inventory()
        object.__setattr__(member_set_mismatch, "members", ())
        cases.append(
            (
                "member_set_mismatch",
                member_set_mismatch,
        ...
        wrong_policy = self.exact_array_backend_deferred_request_inventory()
        object.__setattr__(
            wrong_policy.value_backend_uninit_array,
            "policy",
            "resolve_backend_now",
        )
        cases.append(
        ...
        provenance_mismatch = self.exact_array_backend_deferred_request_inventory()
        other_request = self.exact_array_body_structural_package(
            selected_type_tag="si32",
        ).helper_set_completion.source_backend_uninit_request
        object.__setattr__(
            provenance_mismatch.value_backend_uninit_array,
        ...
        malformed_request_record = self.exact_array_backend_deferred_request_inventory()
        object.__setattr__(
            malformed_request_record.value_backend_uninit_array,
            "source_request_record",
            object(),
        )
        cases.append(
        ...
        for name, inventory, expected_code in cases:
            with self.subTest(name=name):
                self.assertIn(expected_code, diagnostic_codes_for(inventory))

    def test_m90_lowering_completion_package_stage_follows_inventory(self) -> None:
        selection = self.selection_for("lower_generation_size_byte_branch_chain")
        item, envelope = self.size_byte_branch_chain_item_and_envelope("si32")
        skeleton = self.exact_array_body_skeleton_for_envelope(envelope)
        ...
        plan = result.unwrap()
        implementation = plan.implementations_by_candidate_id[item.candidate_id]
        self.assertEqual(len(implementation.array_lowering_completion_packages), 1)
        package = implementation.array_body_structural_packages[0]
        inventory = implementation.array_backend_deferred_request_inventories[0]
        completion = implementation.array_lowering_completion_packages[0]
        self.assertIs(completion.source_package, package)
        self.assertIs(completion.source_inventory, inventory)
        self.assertIs(
            completion.value_backend_uninit_array_dependency.source_inventory_member,
        ...
        implementations_with_completion = tuple(
            lowered.candidate_id
            for lowered in plan.implementations
            if lowered.array_lowering_completion_packages
        )
        self.assertEqual(implementations_with_completion, (item.candidate_id,))

    def test_m90_exact_array_body_pipeline_snapshot_preserves_completion_identity(
        self,
    ) -> None:
        pipeline = self.exact_array_initialization_stage_pipeline("si32")
        ...
        self.assertTrue(pipeline.is_ok, pipeline.diagnostics)
        result = pipeline.unwrap()
        self.assertEqual(len(result.array_lowering_completion_packages), 1)
        inventory = result.array_backend_deferred_request_inventories[0]
        completion = result.array_lowering_completion_packages[0]
        self.assertIs(completion.source_inventory, inventory)
        self.assertIs(result.stages[-3].output, completion)
        snapshot = result.pipeline_snapshot
        self.assertEqual(
            snapshot.steps[-3].stage_name,
            "array_lowering_completion_package",
        )
        ...
        self.assertIs(snapshot.steps[-3].produced_fact.value, completion)
        self.assertEqual(snapshot.steps[-3].produced_fact.key, completion.key)
        self.assertEqual(
            snapshot.steps[-3].depends_on,
            ("array_backend_deferred_request_inventory",),
        )

    def test_m90_lowering_completion_package_is_deterministic_for_reordered_inputs(
        self,
    ) -> None:
        selection = self.selection_for("lower_generation_size_byte_branch_chain")
        baseline = lower_candidates(selection)
        ...
        def completion_keys(plan) -> dict[str, tuple[tuple[object, ...], ...]]:
            return {
                implementation.candidate_id: tuple(
                    package.key
                    for package in implementation.array_lowering_completion_packages
                )
                for implementation in plan.unwrap().implementations
            }

        self.assertEqual(completion_keys(first), completion_keys(second))

    def test_m90_lowering_completion_package_module_import_boundary(self) -> None:
        forbidden_exact_modules = (
            "tslgen.lowering.boundary",
            "tslgen.lowering",
            "tslgen.lowering._array_body_pipeline",
        ...
        imported_forbidden: list[str] = []
        tree = ast.parse(inspect.getsource(lowering_array_body_completion_package))
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                for alias in node.names:
                    if alias.name in forbidden_exact_modules or alias.name.startswith(
        ...
        self.assertEqual(imported_forbidden, [])
        source = inspect.getsource(lowering_array_body_completion_package)
        self.assertNotIn("backend_map", source)
        self.assertNotIn("render", source)
        self.assertNotIn("open(", source)
        self.assertIs(
            lowering_boundary.lower_exact_array_lowering_completion_package,
            lowering_array_body_completion_package.lower_exact_array_lowering_completion_package,
        )
        self.assertIs(
            lower_exact_array_lowering_completion_package,
            lowering_array_body_completion_package.lower_exact_array_lowering_completion_package,
        ...
        self.assertIs(
            lowering_boundary.ExactArrayLoweringCompletionPackageIr,
            lowering_array_body_completion_package.ExactArrayLoweringCompletionPackageIr,
        )
        self.assertIs(
    ...
    def test_m91_stage_assembly_module_preserves_pipeline_snapshot_contract(
        ...
        expected_stage_helper_names = (
            ...
            "_array_backend_handoff_request_stage",
    ...
    def test_m91_pipeline_ownership_modules_import_boundary_and_line_counts(
        ...
        self.assertLessEqual(line_counts["tslgen.lowering.boundary"], 1300)
    ...
    def test_m92_backend_handoff_lowers_m90_sources(self) -> None:
        completion = self.exact_array_lowering_completion_package(
            selected_type_tag="si16",
        )

        class M90LoweredImplementationSource:
            def __init__(
                self,
                accepted_completion: ExactArrayLoweringCompletionPackageIr,
            ) -> None:
                self.array_lowering_completion_packages = (accepted_completion,)
        ...
        sources = (
            ...
            LoweredImplementation(
                candidate_id=completion.candidate_id,
                status="lowered",
                array_lowering_completion_packages=(completion,),
            ),
            M90LoweredImplementationSource(completion),
        )

        for source in sources:
            with self.subTest(source=type(source).__name__):
                result = lower_exact_array_backend_handoff_request(
                    source,
                    GenerationContext(
                        selected_candidate_id=completion.candidate_id,
                        selected_type_tag=completion.selected_type_tag,
                ...
                self.assertTrue(result.is_ok, result.diagnostics)
                handoff = result.unwrap()
                dependency_request = handoff.value_backend_uninit_array_request
                completion_dependency = (
                    completion.value_backend_uninit_array_dependency
                )
                self.assertIsInstance(handoff, ExactArrayBackendHandoffRequestIr)
                self.assertIsInstance(
                    dependency_request,
                    ExactArrayBackendHandoffUnresolvedDependencyRequestIr,
                )
                self.assertIs(handoff.source_completion_package, completion)
                self.assertIs(handoff.source_package, completion.source_package)
                self.assertIs(handoff.source_inventory, completion.source_inventory)
                self.assertEqual(handoff.source_location, completion.source_location)
                self.assertEqual(handoff.candidate_id, completion.candidate_id)
                self.assertEqual(
                    handoff.target_extension,
                    completion.target_extension,
                )
                self.assertEqual(
                    handoff.source_extension,
                    completion.source_extension,
                )
                self.assertEqual(
                    handoff.selected_type_tag,
                ...
                self.assertEqual(
                    handoff.unresolved_dependency_requests,
                    (dependency_request,),
                )
                self.assertEqual(
                    dependency_request.kind,
                    "value_backend_uninit_array",
                )
                self.assertEqual(dependency_request.request_kind, "backend_value")
                self.assertEqual(
                    dependency_request.policy,
                    "deferred_backend_value",
                )
                self.assertIs(
                    dependency_request.source_completion_dependency,
                    completion_dependency,
                )
                self.assertIs(
                    dependency_request.source_inventory_member,
                    completion_dependency.source_inventory_member,
                )
                self.assertIs(
                    dependency_request.source_deferred_backend_uninit,
                    completion_dependency.source_deferred_backend_uninit,
                )
                self.assertIs(
                    dependency_request.source_request_record,
                    completion_dependency.source_request_record,
                )
                self.assertFalse(hasattr(handoff, "backend_translation"))
                self.assertFalse(hasattr(handoff, "renderer_value"))
                self.assertFalse(hasattr(handoff, "stage9_plan"))
                self.assertFalse(hasattr(dependency_request, "resolved_backend_value"))
                self.assertFalse(hasattr(dependency_request, "backend_map_entry"))
                self.assertFalse(hasattr(dependency_request, "artifact_path"))

    def test_m92_backend_handoff_reports_source_diagnostics(self) -> None:
        completion = self.exact_array_lowering_completion_package()

        class RuntimeSource:
            def __init__(self, packages: object = (completion,)) -> None:
                self.array_lowering_completion_packages = packages
        ...
        cases = (
            (
                ...
                GenerationLoweringStage(
                    stage="array_backend_deferred_request_inventory",
                    output=completion.source_inventory,
                ),
            ...
            (
                "non_tuple_completion",
                RuntimeSource(packages=[completion]),
                "TSL-LOWER-ARRAY-BACKEND-HANDOFF-SOURCE-UNSUPPORTED",
                "tuple",
            ),
        )
        ...
        for name, source, code, message in cases:
            with self.subTest(name=name):
                result = lower_exact_array_backend_handoff_request(source)
        ...
        assert_diagnostic(
            self,
            result.diagnostics[0],
            code="TSL-LOWER-ARRAY-BACKEND-HANDOFF-CONTEXT-MISMATCH",
            severity="error",
            path="tsldata/primitives/load_store/array.tsl",
            line=105,
        )

    def test_m92_backend_handoff_reports_boundary_diagnostics(self) -> None:
        def diagnostic_codes_for(
            completion: ExactArrayLoweringCompletionPackageIr,
        ) -> set[str]:
            result = lower_exact_array_backend_handoff_request(completion)
            self.assertFalse(result.is_ok)
            for diagnostic in result.diagnostics:
        ...
        cases: list[
            tuple[str, ExactArrayLoweringCompletionPackageIr, str]
        ] = []

        source_location_mismatch = self.exact_array_lowering_completion_package()
        object.__setattr__(
            source_location_mismatch,
            "source_location",
            SourceLocation(Path("tsldata/primitives/load_store/array.tsl"), 106, 15),
        ...
        context_mismatch = self.exact_array_lowering_completion_package()
        object.__setattr__(context_mismatch, "target_extension", "neon")
        cases.append(
            (
                "context_mismatch",
        ...
        dependency_set_mismatch = self.exact_array_lowering_completion_package()
        object.__setattr__(dependency_set_mismatch, "unresolved_dependencies", ())
        cases.append(
            (
        ...
        wrong_policy = self.exact_array_lowering_completion_package()
        object.__setattr__(
            wrong_policy.value_backend_uninit_array_dependency,
            "policy",
            "resolve_backend_now",
        )
        cases.append(
        ...
        provenance_mismatch = self.exact_array_lowering_completion_package()
        other_request = self.exact_array_body_structural_package(
            selected_type_tag="si32",
        ).helper_set_completion.source_backend_uninit_request
        object.__setattr__(
            provenance_mismatch.value_backend_uninit_array_dependency,
            "source_request_record",
            other_request,
        )
        cases.append(
        ...
        malformed_dependency = self.exact_array_lowering_completion_package()
        object.__setattr__(
            malformed_dependency,
            "value_backend_uninit_array_dependency",
            object(),
        )
        cases.append(
            (
        ...
        for name, completion, expected_code in cases:
            with self.subTest(name=name):
                self.assertIn(expected_code, diagnostic_codes_for(completion))

    def test_m92_backend_handoff_stage_follows_completion(self) -> None:
        selection = self.selection_for("lower_generation_size_byte_branch_chain")
        item, envelope = self.size_byte_branch_chain_item_and_envelope("si32")
        skeleton = self.exact_array_body_skeleton_for_envelope(envelope)
        ...
        inventory = implementation.array_backend_deferred_request_inventories[0]
        completion = implementation.array_lowering_completion_packages[0]
        handoff = implementation.array_backend_handoff_requests[0]
        dependency_request = handoff.value_backend_uninit_array_request
        self.assertIs(handoff.source_completion_package, completion)
        self.assertIs(handoff.source_package, package)
        self.assertIs(handoff.source_inventory, inventory)
        self.assertIs(
            dependency_request.source_completion_dependency,
            completion.value_backend_uninit_array_dependency,
        )
        ...
        implementations_with_handoff = tuple(
            lowered.candidate_id
            for lowered in plan.implementations
            if lowered.array_backend_handoff_requests
        )
        self.assertEqual(implementations_with_handoff, (item.candidate_id,))

    def test_m92_exact_array_body_pipeline_snapshot_preserves_handoff_identity(
        self,
    ) -> None:
        pipeline = self.exact_array_initialization_stage_pipeline("si32")
        ...
        self.assertTrue(pipeline.is_ok, pipeline.diagnostics)
        result = pipeline.unwrap()
        self.assertEqual(len(result.array_backend_handoff_requests), 1)
        completion = result.array_lowering_completion_packages[0]
        handoff = result.array_backend_handoff_requests[0]
        self.assertIs(handoff.source_completion_package, completion)
        self.assertIs(result.stages[-2].output, handoff)
        snapshot = result.pipeline_snapshot
        self.assertEqual(snapshot.steps[-2].stage_name, "array_backend_handoff_request")
        self.assertIs(snapshot.steps[-2].stage, result.stages[-2])
        self.assertEqual(
            snapshot.steps[-2].produced_fact.kind,
            "array_backend_handoff_request",
        )
        self.assertIs(snapshot.steps[-2].produced_fact.value, handoff)
        self.assertEqual(snapshot.steps[-2].produced_fact.key, handoff.key)
        self.assertEqual(
            snapshot.steps[-2].depends_on,
            ("array_lowering_completion_package",),
        )

    def test_m92_backend_handoff_request_is_deterministic_for_reordered_inputs(
        self,
    ) -> None:
        selection = self.selection_for("lower_generation_size_byte_branch_chain")
        baseline = lower_candidates(selection)
        ...
        def handoff_keys(plan) -> dict[str, tuple[tuple[object, ...], ...]]:
            return {
                implementation.candidate_id: tuple(
                    handoff.key
                    for handoff in implementation.array_backend_handoff_requests
                )
                for implementation in plan.unwrap().implementations
            }

        self.assertEqual(handoff_keys(first), handoff_keys(second))

    def test_m92_backend_handoff_module_import_boundary(self) -> None:
        forbidden_exact_modules = (
            "tslgen.lowering.boundary",
            "tslgen.lowering",
            "tslgen.lowering._array_body_pipeline",
        ...
        imported_forbidden: list[str] = []
        tree = ast.parse(inspect.getsource(lowering_array_body_backend_handoff))
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                for alias in node.names:
                    if alias.name in forbidden_exact_modules or alias.name.startswith(
        ...
        self.assertEqual(imported_forbidden, [])
        source = inspect.getsource(lowering_array_body_backend_handoff)
        self.assertNotIn("backend_map", source)
        self.assertNotIn("render", source)
        self.assertNotIn("open(", source)
        self.assertNotIn("Stage9", source)
        self.assertIs(
            lowering_boundary.lower_exact_array_backend_handoff_request,
            lowering_array_body_backend_handoff.lower_exact_array_backend_handoff_request,
        )
        self.assertIs(
            lower_exact_array_backend_handoff_request,
            lowering_array_body_backend_handoff.lower_exact_array_backend_handoff_request,
        ...
        self.assertIs(
            lowering_boundary.ExactArrayBackendHandoffRequestIr,
            lowering_array_body_backend_handoff.ExactArrayBackendHandoffRequestIr,
        )
        self.assertIs(
            lowering_boundary.ExactArrayBackendHandoffUnresolvedDependencyRequestIr,
            lowering_array_body_backend_handoff.ExactArrayBackendHandoffUnresolvedDependencyRequestIr,
        )

    def test_m93_mini_tsil_leaf_return_package_sources_preserve_identity(
        self,
    ) -> None:
        location = SourceLocation(Path("tsldata/primitives/arithmetic/fundamental.tsl"), 31, 9)
        statement = TsilReturnStatement(
            TsilBinaryExpression(
                operator="+",
                left=TsilParameterReference("left"),
        ...
        direct = lower_lowering_operation_package(
            statement,
            candidate_id="candidate-1",
            source_location=location,
        )
        staged = lower_lowering_operation_package(
            stage,
            candidate_id="candidate-1",
            source_location=location,
        )
        from_container = lower_lowering_operation_package(
            container,
            source_location=location,
        )
        ...
        for result in (direct, staged, from_container):
            package = result.unwrap()
            self.assertEqual(package.source_family, "mini_tsil_leaf_return")
            self.assertEqual(package.candidate_id, "candidate-1")
            self.assertEqual(package.source_location, location)
            self.assertIsInstance(
                package.mini_tsil_leaf_return,
                MiniTsilLeafReturnOperationPackageEntryIr,
            )
            assert package.mini_tsil_leaf_return is not None
            self.assertIs(
                package.mini_tsil_leaf_return.source_statement,
                statement,
            )
            self.assertEqual(package.exact_array_backend_handoff, None)
            self.assertEqual(package.key, direct.unwrap().key)

    def test_m93_exact_array_backend_handoff_package_sources_preserve_identity(
        self,
    ) -> None:
        handoff = self.exact_array_backend_handoff_request()
        stage = GenerationLoweringStage(
        ...
        container = LoweredImplementation(
            ...
            status="lowered",
            array_backend_handoff_requests=(handoff,),
        )

        direct = lower_lowering_operation_package(handoff)
        ...
        for result in (direct, staged, from_container):
            package = result.unwrap()
            self.assertEqual(package.source_family, "exact_array_backend_handoff")
            self.assertEqual(package.candidate_id, handoff.candidate_id)
            self.assertEqual(package.source_location, handoff.source_location)
            self.assertIsInstance(
                package.exact_array_backend_handoff,
                ExactArrayBackendHandoffOperationPackageEntryIr,
            )
            assert package.exact_array_backend_handoff is not None
            self.assertIs(
                package.exact_array_backend_handoff.source_request,
                handoff,
            )
            self.assertIs(handoff.source_package, handoff.source_completion_package.source_package)
            self.assertIs(handoff.source_inventory, handoff.source_completion_package.source_inventory)
    ...
    def test_m93_lower_candidates_exposes_operation_package_stages(
        ...
        mini = lower_candidates(self.selection_for("lower_intrin_add"))
        self.assertTrue(mini.is_ok, mini.diagnostics)
        mini_impl = mini.unwrap().implementations[0]
        self.assertEqual(len(mini_impl.operation_packages), 1)
        mini_package = mini_impl.operation_packages[0]
        self.assertEqual(mini_package.source_family, "mini_tsil_leaf_return")
        assert mini_package.mini_tsil_leaf_return is not None
        self.assertIs(
            mini_package.mini_tsil_leaf_return.source_statement,
            mini_impl.statements[0],
        )
        self.assertEqual(
            tuple(stage.stage for stage in mini_impl.generation_stages),
            (
                "selected_body_lowering",
                "lowering_operation_package",
                "lowering_completion_manifest",
        ...
        self.assertIs(
            exact_package.exact_array_backend_handoff.source_request,
    ...
    def test_m93_exact_array_pipeline_snapshot_carries_operation_package(
        ...
        self.assertTrue(pipeline.is_ok, pipeline.diagnostics)
        result = pipeline.unwrap()
        self.assertEqual(len(result.operation_packages), 1)
        package = result.operation_packages[0]
        assert package.exact_array_backend_handoff is not None
        self.assertIs(
            package.exact_array_backend_handoff.source_request,
            result.array_backend_handoff_requests[0],
        )
        self.assertIs(result.stages[-1].output, package)
        snapshot = result.pipeline_snapshot
        self.assertEqual(snapshot.steps[-1].stage_name, "lowering_operation_package")
    ...
    def test_m93_operation_package_reports_boundary_diagnostics(self) -> None:
        location = SourceLocation(Path("operation.tsl"), 7, 11)
        other_location = SourceLocation(Path("operation.tsl"), 8, 3)
        statement = TsilReturnStatement(
            TsilBinaryExpression(
                operator="+",
                left=TsilParameterReference("left"),
        ...
        other_statement = TsilReturnStatement(
            TsilBinaryExpression(
                operator="+",
        ...
        handoff = self.exact_array_backend_handoff_request()
        package = lower_lowering_operation_package(handoff).unwrap()
        non_m86_statement = TsilReturnStatement(
            TsilIntrinsicComposeExpression(
                intrinsic="sub",
                arguments=(
        ...
        cases: tuple[
            ...
            (
                ...
                LoweredImplementation(
                    candidate_id="candidate-1",
                    status="lowered",
                    statements=(statement, other_statement),
                ),
                {"source_location": location},
            ...
            (
                ...
                type(
                    "MalformedM93Source",
                    (),
                    {"candidate_id": "candidate-1", "statements": [statement]},
                )(),
                {},
    ...
    def test_m93_operation_package_reports_handoff_provenance_mismatches(
        ...
        broken_package = self.exact_array_backend_handoff_request()
        object.__setattr__(broken_package, "source_package", object())

        provenance = lower_lowering_operation_package(broken_package)

        self.assertFalse(provenance.is_ok)
        assert_diagnostic(
            self,
            provenance.diagnostics[0],
        ...
        object.__setattr__(
            broken_dependency.value_backend_uninit_array_request,
            "source_request_record",
            object(),
        )

        dependency = lower_lowering_operation_package(broken_dependency)

        self.assertFalse(dependency.is_ok)
        assert_diagnostic(
            self,
    ...
    def test_m93_operation_package_determinism_for_reordered_inputs(self) -> None:
        ...
        def package_keys(plan) -> dict[str, tuple[tuple[object, ...], ...]]:
            return {
                implementation.candidate_id: tuple(
                    package.key for package in implementation.operation_packages
                )
                for implementation in plan.unwrap().implementations
            }
    ...
    def test_m93_operation_package_import_boundary_and_public_facade(self) -> None:
        ...
        forbidden_exact_modules = (
            ...
            "tslgen.lowering._array_body_pipeline",
            "tslgen.lowering._array_body_sources",
            "tslgen.lowering._array_body_stage_assembly",
        )
        forbidden_prefixes = (
            "tslgen.backends",
            "tslgen.rendering",
        ...
        self.assertIs(
            lowering_boundary.LoweringOperationPackageIr,
        ...
        self.assertIs(
            lowering_boundary.MiniTsilLeafReturnOperationPackageEntryIr,
        ...
        self.assertIn(
            "lowering_operation_package",
            get_args(lowering_pipeline.ExactArrayBodyPipelineStageName.__value__),
        )
        self.assertEqual(
    ...
    def test_m96_completion_manifest_constructs_current_package_families(
        self,
    ) -> None:
        location = SourceLocation(Path("manifest.tsl"), 7, 11)
        statement = TsilReturnStatement(
            TsilBinaryExpression(
                operator="+",
                left=TsilParameterReference("left"),
        ...
        cases = (
            (mini_package, "mini_tsil_leaf_return", False),
            (exact_package, "exact_array_backend_handoff", True),
            (selected_package, "selected_body_direct_intrinsic", False),
        )
        for package, source_family, has_dependency in cases:
            with self.subTest(source_family=source_family):
                result = lowering_completion_manifest.lower_stage8_lowering_completion_manifest(
                    package,
                )

                self.assertTrue(result.is_ok, result.diagnostics)
                manifest = result.unwrap()
                self.assertEqual(manifest.candidate_id, package.candidate_id)
                self.assertEqual(manifest.source_packages, (package,))
                self.assertEqual(manifest.package_families, (source_family,))
                self.assertIs(manifest.package_records[0].source_package, package)
                self.assertEqual(
                    manifest.has_unresolved_backend_dependencies,
                    has_dependency,
                )
                if has_dependency:
                    assert package.exact_array_backend_handoff is not None
                    request = package.exact_array_backend_handoff.source_request
                    self.assertIs(
                        manifest.unresolved_dependency_records[0].source_package,
                        package,
                    )
                    self.assertIs(
    ...
    def test_m96_completion_manifest_mixed_candidate_preserves_identity(
        self,
    ) -> None:
        selection = self.selection_for("lower_generation_size_byte_branch_chain")
        item, envelope = self.size_byte_branch_chain_item_and_envelope("si32")
        ...
        self.assertEqual(len(implementation.operation_packages), 2)
        self.assertEqual(len(implementation.lowering_completion_manifests), 1)
        manifest = implementation.lowering_completion_manifests[0]
        expected_packages = tuple(
            sorted(implementation.operation_packages, key=lambda package: package.key)
        )
        self.assertEqual(
            tuple(id(package) for package in manifest.source_packages),
            tuple(id(package) for package in expected_packages),
        )
        self.assertEqual(
            tuple(id(record.source_package) for record in manifest.package_records),
            tuple(id(package) for package in expected_packages),
        )
        self.assertEqual(
        ...
        exact_package = implementation.operation_packages[1]
        assert exact_package.exact_array_backend_handoff is not None
        handoff = exact_package.exact_array_backend_handoff.source_request
        self.assertIs(
            manifest.unresolved_dependency_records[0].source_dependency_request,
            handoff.value_backend_uninit_array_request,
        )
        self.assertIs(
            manifest.unresolved_dependency_records[
                0
            ].source_dependency_request.source_completion_dependency,
            handoff.source_completion_package.value_backend_uninit_array_dependency,
        )

    def test_m96_completion_manifest_reports_boundary_diagnostics(self) -> None:
        location = SourceLocation(Path("manifest.tsl"), 7, 11)
        other_location = SourceLocation(Path("manifest.tsl"), 8, 3)
        statement = TsilReturnStatement(
            TsilBinaryExpression(
                operator="+",
                left=TsilParameterReference("left"),
        ...
        malformed = object.__new__(LoweringOperationPackageIr)
        object.__setattr__(malformed, "candidate_id", "candidate-1")
        object.__setattr__(malformed, "source_location", location)
        object.__setattr__(malformed, "source_family", "mini_tsil_leaf_return")
        object.__setattr__(malformed, "mini_tsil_leaf_return", None)
        object.__setattr__(malformed, "exact_array_backend_handoff", None)
        object.__setattr__(malformed, "selected_body_direct_intrinsic", None)
        wrong_family = lower_lowering_operation_package(
            self.exact_array_backend_handoff_request(),
        ).unwrap()
        object.__setattr__(wrong_family, "source_family", "mini_tsil_leaf_return")
        wrong_candidate = lower_lowering_operation_package(
            statement,
            candidate_id="candidate-1",
            source_location=location,
        ).unwrap()
        object.__setattr__(wrong_candidate, "candidate_id", "other-candidate")
        wrong_location = lower_lowering_operation_package(
            statement,
            candidate_id="candidate-1",
            source_location=location,
        ).unwrap()
        object.__setattr__(wrong_location, "source_location", other_location)
        ambiguous = type(
            "AmbiguousM96Source",
            (),
            {
        ...
        for name, source, kwargs, code in cases:
            with self.subTest(name=name):
                ...
                if name in {"candidate", "entry_context"}:
                    assert_diagnostic(
                        self,
                        result.diagnostics[0],
                        code=code,
                    ...
                    self.assertIn("candidate", result.diagnostics[0].message)
                if name in {"location", "entry_location"}:
                    expected_line = 8 if name == "entry_location" else 7
                    expected_column = 3 if name == "entry_location" else 11
                    assert_diagnostic(
                        self,
                        result.diagnostics[0],
                        code=code,
                    ...
                    self.assertIn("source location", result.diagnostics[0].message)

    def test_m96_completion_manifest_rejects_mixed_valid_candidate_packages(
        self,
    ) -> None:
        location = SourceLocation(Path("manifest.tsl"), 7, 11)
        other_location = SourceLocation(Path("manifest.tsl"), 8, 3)
        ...
        other_package = lower_lowering_operation_package(
            statement,
        ...
        assert_diagnostic(
            self,
            result.diagnostics[0],
            code="TSL-LOWER-COMPLETION-MANIFEST-CONTEXT-MISMATCH",
            severity="error",
            path="manifest.tsl",
            line=7,
            column=11,
        )
        self.assertIn("share one candidate id", result.diagnostics[0].message)

    def test_m96_completion_manifest_reports_dependency_provenance_mismatch(
        self,
    ) -> None:
        package = lower_lowering_operation_package(
            self.exact_array_backend_handoff_request(),
        ...
        object.__setattr__(dependency, "source_request_record", object())
        ...
        assert_diagnostic(
            self,
            result.diagnostics[0],
            code="TSL-LOWER-COMPLETION-MANIFEST-DEPENDENCY-PROVENANCE-MISMATCH",
            severity="error",
            path=dependency.source_location.path.as_posix(),
            line=dependency.source_location.line,
            column=dependency.source_location.column,
        )
        self.assertIn("M90/M89/M67", result.diagnostics[0].message)

    def test_m96_completion_manifest_rejects_copied_dependency_requests(
        self,
    ) -> None:
        package = lower_lowering_operation_package(
            self.exact_array_backend_handoff_request(),
        ...
        copied_dependency = replace(dependency)
        self.assertEqual(copied_dependency, dependency)
        self.assertIsNot(copied_dependency, dependency)

        with self.assertRaisesRegex(ValueError, "object identity"):
            lowering_completion_manifest.Stage8LoweringCompletionManifestUnresolvedDependencyRecordIr(
                source_package=package,
                source_dependency_request=copied_dependency,
            )
        ...
        object.__setattr__(
            manifest.unresolved_dependency_records[0],
            "source_dependency_request",
            copied_dependency,
        )
        ...
        self.assertIn("object identity", result.diagnostics[0].message)

    def test_m96_completion_manifest_is_deterministic_for_reordered_packages(
        self,
    ) -> None:
        handoff = self.exact_array_backend_handoff_request(selected_type_tag="si32")
        exact_package = lower_lowering_operation_package(handoff).unwrap()
        selected_package = lower_lowering_operation_package(
            self.selected_body_envelope(selected_type_tag="si32"),
        ).unwrap()

        first = lowering_completion_manifest.lower_stage8_lowering_completion_manifest(
            (exact_package, selected_package),
        )
        second = lowering_completion_manifest.lower_stage8_lowering_completion_manifest(
            (selected_package, exact_package),
        )

        self.assertTrue(first.is_ok, first.diagnostics)
        self.assertTrue(second.is_ok, second.diagnostics)
        self.assertEqual(first.unwrap().key, second.unwrap().key)
        self.assertEqual(first.unwrap().source_packages, second.unwrap().source_packages)
        self.assertEqual(
            tuple(record.key for record in first.unwrap().package_records),
            tuple(record.key for record in second.unwrap().package_records),
        )

    def test_m96_completion_manifest_stage_contract_and_pipeline_integration(
        self,
    ) -> None:
        mini = lower_candidates(self.selection_for("lower_intrin_add"))
        self.assertTrue(mini.is_ok, mini.diagnostics)
        ...
        self.assertEqual(pipeline.stages[-1].stage, "lowering_operation_package")
        self.assertEqual(
            pipeline.pipeline_snapshot.steps[-1].stage_name,
            "lowering_operation_package",
        )
        self.assertNotIn(
            "lowering_completion_manifest",
    ...
    def test_m96_completion_manifest_import_boundary_and_line_counts(self) -> None:
        tree = ast.parse(inspect.getsource(lowering_completion_manifest))
        forbidden_exact_modules = {
            "tslgen.lowering.boundary",
            "tslgen.lowering",
        }
        ...
        self.assertEqual(imported_forbidden, [])
        source = inspect.getsource(lowering_completion_manifest)
        for forbidden in (
            "backend_map",
            "catalog",
            "Stage9",
    ...
    def test_m97_completion_gap_inventory_constructs_supported_manifest_facts(
        self,
    ) -> None:
        location = SourceLocation(Path("gap-inventory.tsl"), 7, 11)
        statement = TsilReturnStatement(
            TsilBinaryExpression(
                operator="+",
                left=TsilParameterReference("left"),
        ...
        cases = (
            ((mini_package,), "no_known_gap", 0),
            ((selected_package,), "no_known_gap", 0),
            (
                (exact_package,),
        ...
        for packages, state, gap_count in cases:
            with self.subTest(state=state, package_count=len(packages)):
                manifest = lowering_completion_manifest.lower_stage8_lowering_completion_manifest(
                    packages,
                ).unwrap()

                result = lowering_completion_gap_inventory.lower_stage8_lowering_completion_gap_inventory(
                    manifest,
                )

                self.assertTrue(result.is_ok, result.diagnostics)
                inventory = result.unwrap()
                self.assertEqual(inventory.candidate_id, manifest.candidate_id)
                self.assertEqual(inventory.inventory_state, state)
                self.assertIs(inventory.source_manifest, manifest)
                self.assertEqual(len(inventory.gap_records), gap_count)
                self.assertEqual(inventory.has_known_gaps, bool(gap_count))
                if gap_count:
                    gap = inventory.gap_records[0]
                    dependency_record = manifest.unresolved_dependency_records[0]
                    self.assertIs(gap.source_manifest, manifest)
                    self.assertIs(
                        gap.source_unresolved_dependency_record,
                        dependency_record,
                    )
                    expected_package_record = next(
                        package_record
                        for package_record in manifest.package_records
                        if package_record.source_package
                        is dependency_record.source_package
                    )
                    self.assertIs(gap.source_package_record, expected_package_record)
                    self.assertIs(
                        gap.source_dependency_request,
                        dependency_record.source_dependency_request,
                    )
                    self.assertIs(gap.source_package, dependency_record.source_package)
                    self.assertIs(
                        gap.source_package_record.source_package,
                        dependency_record.source_package,
                    )

    def test_m97_completion_gap_inventory_determinism_and_stage_order(
        self,
    ) -> None:
        handoff = self.exact_array_backend_handoff_request(selected_type_tag="si32")
        exact_package = lower_lowering_operation_package(handoff).unwrap()
        ...
        first_manifest = lowering_completion_manifest.lower_stage8_lowering_completion_manifest(
            (exact_package, selected_package),
        ).unwrap()
        second_manifest = lowering_completion_manifest.lower_stage8_lowering_completion_manifest(
            (selected_package, exact_package),
        ).unwrap()

        first = lowering_completion_gap_inventory.lower_stage8_lowering_completion_gap_inventory(
            first_manifest,
        )
        second = lowering_completion_gap_inventory.lower_stage8_lowering_completion_gap_inventory(
            second_manifest,
        )
        ...
        self.assertEqual(first.unwrap().key, second.unwrap().key)
        self.assertEqual(
            tuple(record.key for record in first.unwrap().gap_records),
            tuple(record.key for record in second.unwrap().gap_records),
        )
    ...
    def test_m97_completion_gap_inventory_reports_boundary_diagnostics(
        self,
    ) -> None:
        location = SourceLocation(Path("gap-inventory.tsl"), 7, 11)
        other_location = SourceLocation(Path("gap-inventory.tsl"), 8, 3)
        statement = TsilReturnStatement(
            TsilBinaryExpression(
                operator="+",
                left=TsilParameterReference("left"),
        ...
        malformed_manifest = object.__new__(
            lowering_completion_manifest.Stage8LoweringCompletionManifestIr
        )
        object.__setattr__(malformed_manifest, "candidate_id", "candidate-1")
        object.__setattr__(malformed_manifest, "source_location", location)
        object.__setattr__(
            malformed_manifest,
            "completion_state",
            "stage8_package_provenance_assembled",
        )
        object.__setattr__(malformed_manifest, "source_packages", ())
        object.__setattr__(malformed_manifest, "package_records", ())
        object.__setattr__(malformed_manifest, "unresolved_dependency_records", ())
        wrong_stage = GenerationLoweringStage(
            stage="lowering_operation_package",
            output=package,
        )
        ...
        cases: tuple[tuple[str, object, dict[str, Any], str], ...] = (
            ...
            (
                "empty",
                type("EmptyM97Source", (), {"lowering_completion_manifests": ()})(),
                {},
            ...
            (
                ...
                type(
                    "MultipleM97Source",
                    (),
                    {"lowering_completion_manifests": (manifest, other_manifest)},
                )(),
                {},
            ...
            (
                ...
                type(
                    "MalformedM97Stage",
                    (),
                    {"stage": "lowering_completion_manifest", "output": object()},
                )(),
                {},
        ...
        for name, source, kwargs, code in cases:
            with self.subTest(name=name):
                ...
                self.assertFalse(result.is_ok)
                assert_diagnostic(self, result.diagnostics[0], code=code, severity="error")
                if name in {"candidate", "location", "malformed_manifest"}:
                    assert_diagnostic(
                        self,
                        result.diagnostics[0],
                        code=code,
                        severity="error",
                        path="gap-inventory.tsl",
                        line=7,
                        column=11,
                    )
                if name == "candidate":
                    self.assertIn(
                        "candidate context must match the accepted manifest",
                        result.diagnostics[0].message,
                    )
                elif name == "location":
                    self.assertIn(
                        "source location must match the accepted manifest",
                        result.diagnostics[0].message,
                    )
                elif name == "malformed_manifest":
                    self.assertIn(
                        "requires at least one accepted",
                        result.diagnostics[0].message,
                    )

    def test_m97_completion_gap_inventory_rejects_copied_manifest_records(
        self,
    ) -> None:
        package = lower_lowering_operation_package(
            self.exact_array_backend_handoff_request(),
        ...
        dependency_record = manifest.unresolved_dependency_records[0]
        copied_dependency_record = replace(dependency_record)
        self.assertEqual(copied_dependency_record, dependency_record)
        self.assertIsNot(copied_dependency_record, dependency_record)

        with self.assertRaisesRegex(ValueError, "object identity"):
            lowering_completion_gap_inventory.Stage8LoweringCompletionGapRecordIr(
                source_manifest=manifest,
                source_package_record=manifest.package_records[0],
                source_package=dependency_record.source_package,
                source_unresolved_dependency_record=copied_dependency_record,
        ...
        self.assertIsNot(copied_manifest, manifest)
        object.__setattr__(inventory, "source_manifest", copied_manifest)
        copied_manifest_result = lowering_completion_gap_inventory.lower_stage8_lowering_completion_gap_inventory(
            inventory,
        )
        self.assertFalse(copied_manifest_result.is_ok)
        assert_diagnostic(
            self,
            copied_manifest_result.diagnostics[0],
            code="TSL-LOWER-COMPLETION-GAP-INVENTORY-PROVENANCE-MISMATCH",
            severity="error",
        )
        self.assertIn("source manifest object identity", copied_manifest_result.diagnostics[0].message)
        object.__setattr__(inventory, "source_manifest", manifest)

        copied_package_record = replace(manifest.package_records[0])
        self.assertEqual(copied_package_record, manifest.package_records[0])
        self.assertIsNot(copied_package_record, manifest.package_records[0])
        object.__setattr__(
            inventory.gap_records[0],
            "source_package_record",
            copied_package_record,
        )
        copied_package_record_result = lowering_completion_gap_inventory.lower_stage8_lowering_completion_gap_inventory(
            inventory,
        )
        self.assertFalse(copied_package_record_result.is_ok)
        assert_diagnostic(
            self,
            copied_package_record_result.diagnostics[0],
            code="TSL-LOWER-COMPLETION-GAP-INVENTORY-PROVENANCE-MISMATCH",
            severity="error",
        )
        self.assertIn("package record object identity", copied_package_record_result.diagnostics[0].message)
        object.__setattr__(
            inventory.gap_records[0],
            "source_package_record",
            manifest.package_records[0],
        )

        copied_request = replace(dependency_record.source_dependency_request)
        self.assertEqual(copied_request, dependency_record.source_dependency_request)
        self.assertIsNot(copied_request, dependency_record.source_dependency_request)
        object.__setattr__(
            inventory.gap_records[0],
            "source_dependency_request",
            copied_request,
    ...
    def test_m97_completion_gap_inventory_import_boundary_and_line_counts(self) -> None:
        tree = ast.parse(inspect.getsource(lowering_completion_gap_inventory))
        forbidden_exact_modules = {
            "tslgen.lowering.boundary",
            "tslgen.lowering",
        }
        ...
        self.assertEqual(imported_forbidden, [])
        source = inspect.getsource(lowering_completion_gap_inventory)
        for forbidden in (
            "backend_map",
            "catalog",
            "Stage9",
