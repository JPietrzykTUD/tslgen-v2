...
_REGISTER_TYPE_BY_TYPE_TAG = {
    "f32": ("__m256", "core::arch::x86_64::__m256"),
    "f64": ("__m256d", "core::arch::x86_64::__m256d"),
    **{
        type_tag: ("__m256i", "core::arch::x86_64::__m256i")
        for type_tag in _INTEGER_TYPE_TAGS
    },
}
_SELECTED_AVX2_UNMASKED_BINARY_ARITHMETIC_MATRIX = tuple(
    SelectedPrimitiveBodyRenderEntry(
        primitive_name=primitive_name,
        selector_path=("avx2", "?i?"),
        extension="avx2",
        type_tag=type_tag,
        function_name=f"{primitive_name}_avx2_{type_tag}",
...
def test_m251_real_avx2_unmasked_binary_arithmetic_matrix_renders_typed_calls() -> None:
    result = _build_real_avx2_unmasked_binary_arithmetic_matrix()

    assert result.diagnostics == ()
    assert result.model is not None
    assert len(result.selections) == len(_SELECTED_AVX2_UNMASKED_BINARY_ARITHMETIC_MATRIX)
    ...
    for primitive_name in ("add", "sub"):
        for type_tag, suffix in _SIGNED_INTEGER_SUFFIX_BY_TYPE_TAG.items():
            _assert_cpp_function(
                ...
                call=f"_mm256_{primitive_name}_{suffix}(left, right)",
            )
            _assert_rust_function(
                rust_profile,
                primitive_name=primitive_name,
                type_tag=type_tag,
                register_type="core::arch::x86_64::__m256i",
                call=f"core::arch::x86_64::_mm256_{primitive_name}_{suffix}(left, right)",
            )
    ...
    for width in ("8", "16", "32", "64"):
        ...
        assert f"_mm256_sub_epu{width}" not in cpp_profile
        assert f"core::arch::x86_64::_mm256_add_epu{width}" not in rust_profile
        assert f"core::arch::x86_64::_mm256_sub_epu{width}" not in rust_profile

    assert "?i?" not in cpp_profile
    assert "?i?" not in rust_profile
    assert "f?" not in cpp_profile
    assert "f?" not in rust_profile
    assert "core::arch::x86_64::core::arch::x86_64" not in rust_profile


def test_m251_real_avx2_unmasked_binary_arithmetic_matrix_is_deterministic() -> None:
    first = _build_real_avx2_unmasked_binary_arithmetic_matrix()
    second = _build_real_avx2_unmasked_binary_arithmetic_matrix()
...
def test_m251_real_avx2_unmasked_binary_arithmetic_matrix_builds_cpp_and_rust(
    tmp_path: Path,
) -> None:
    result = _build_real_avx2_unmasked_binary_arithmetic_matrix()
...
def test_m251_project_pipeline_keeps_binary_arithmetic_slice_generic_and_typed() -> None:
...
def _build_real_avx2_unmasked_binary_arithmetic_matrix() -> SelectedPrimitiveProjectResult:
    return build_primitive_project_artifacts_from_selected_bodies(
        ...
        selected_entries=_SELECTED_AVX2_UNMASKED_BINARY_ARITHMETIC_MATRIX,
    )
...
def _assert_cpp_function(
    ...
    *,
    ...
    assert (
        f"inline {register_type} {primitive_name}_avx2_{type_tag}"
...
def _assert_rust_function(
    ...
    *,
    ...
    function = _function_text(source, f"{primitive_name}_avx2_{type_tag}")
