...
from tslgen.domain.catalog import Catalog, ImplementationBody
...
def test_m254_6_fragment_standalone_inventory_with_empty_tokens(
    tmp_path: Path,
) -> None:
...
def test_m254_6_fragment_emit_return_payload_inventory_with_empty_tokens(
    tmp_path: Path,
) -> None:
    inventory = _fragment_inventory(
        tmp_path,
...
def test_m254_6_fragment_inventory_preserves_source_order_and_continues(
    tmp_path: Path,
) -> None:
    inventory = _fragment_inventory(
        tmp_path,
...
def test_m254_6_fragment_inventory_keeps_call_arguments_opaque(
    tmp_path: Path,
) -> None:
    inventory = _fragment_inventory(
        tmp_path,
...
def test_m254_6_fragment_inventory_reports_unsupported_selector(
    tmp_path: Path,
) -> None:
    inventory = _fragment_inventory(
        tmp_path,
...
def test_m254_6_fragment_inventory_reports_unknown_target(tmp_path: Path) -> None:
    inventory = _fragment_inventory(
        tmp_path,
        "call<primitive=missing[Vec]>(left, right)",
    )
...
def test_m254_6_fragment_inventory_reports_missing_target_implementation(
    tmp_path: Path,
) -> None:
    inventory = _fragment_inventory(
        tmp_path,
...
def test_m254_6_fragment_inventory_reports_arity_mismatch(tmp_path: Path) -> None:
    inventory = _fragment_inventory(
        tmp_path,
...
def test_m254_6_token_only_inventory_fallback_remains_available(
    tmp_path: Path,
) -> None:
    ...
    selected = _selected_without_fragments(selected)
...
def test_m254_6_primitive_call_inventory_guardrails() -> None:
    primitive_calls_text = (
        SRC / "lowering" / "primitive_calls.py"
    ).read_text(encoding="utf-8")
    project_text = primitive_calls_text + (
        SRC / "lowering" / "source_body_fragments.py"
    ).read_text(encoding="utf-8")

    assert "extract_primitive_call_directives" in primitive_calls_text
    assert "_primitive_calls_in_source_order_from_body_tokens" in primitive_calls_text
    assert "selected.implementation.body.tokens" in primitive_calls_text
    ...
    assert not any(text in project_text for text in forbidden)
...
def _fragment_inventory(
    ...
    *,
    ...
    selected = _selected_with_empty_body_tokens(selected)
...
def _selected_with_empty_body_tokens(
    ...
    implementation = replace(
        ...
        body=ImplementationBody(
            tokens=(),
            source=selected.implementation.body.source,
        ),
    )
    assert implementation.body.tokens == ()
    return replace(selected, implementation=implementation)


def _selected_without_fragments(
    selected: SelectedImplementation,
) -> SelectedImplementation:
    implementation = replace(selected.implementation, source_body_fragments=None)
    return replace(selected, implementation=implementation)
