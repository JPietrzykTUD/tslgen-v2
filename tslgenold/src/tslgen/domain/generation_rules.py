...
type ScalarSizeBytesGenerationTagStatus = Literal["selected", "unsupported", "unknown"]
...
SELECTED_SCALAR_SIZE_BYTES_GENERATION_TYPE_TAGS: tuple[str, ...] = (
    "si8",
    "ui8",
    "si16",
    "ui16",
...
_SELECTED_SCALAR_SIZE_BYTES_TAGS = frozenset(
...
_TSDATA_TYPES_LINE_BY_SELECTED_SCALAR_SIZE_TAG = FrozenMap(
    {
        "si8": 2,
        "si16": 3,
        "si32": 4,
...
@dataclass(frozen=True, slots=True)
class ScalarSizeBytesGenerationRule:
    type_tag: str
    size_bytes: int

    def __post_init__(self) -> None:
        if not self.type_tag:
            raise ValueError("scalar size-bytes generation type_tag is required")
        if (
            isinstance(self.size_bytes, bool)
            or not isinstance(self.size_bytes, int)
            or self.size_bytes < 1
        ):
            raise ValueError("scalar size-bytes generation value must be positive")

    @property
    def key(self) -> tuple[str, int]:
        return (self.type_tag, self.size_bytes)


SCALAR_SIZE_BYTES_GENERATION_RULES: tuple[ScalarSizeBytesGenerationRule, ...] = (
...
@dataclass(frozen=True, slots=True)
class ScalarSizeBytesGenerationRuleSet:
    rules: tuple[ScalarSizeBytesGenerationRule, ...]
    rules_by_type_tag: FrozenMap[str, ScalarSizeBytesGenerationRule] = field(
        init=False
    )
    ...
    def __post_init__(self) -> None:
        rules = tuple(
            sorted(
                self.rules,
                key=lambda rule: _scalar_size_tag_sort_key(rule.type_tag),
            )
        )
        object.__setattr__(self, "rules", rules)
        object.__setattr__(
    ...
    def rule_for(self, type_tag: str) -> ScalarSizeBytesGenerationRule | None:
        return self.rules_by_type_tag.get(type_tag)
...
def build_scalar_size_bytes_generation_rule_set(
    type_groups: tuple[TypeGroup, ...],
    *,
    selected_type_tags: tuple[
        str, ...
    ] = SELECTED_SCALAR_SIZE_BYTES_GENERATION_TYPE_TAGS,
) -> Result[ScalarSizeBytesGenerationRuleSet]:
    groups_by_name = FrozenMap((group.name, group) for group in type_groups)
    selected_tags = _normalize_scalar_size_tags(selected_type_tags)
    diagnostics: list[Diagnostic] = []
    ...
    for type_tag in selected_tags:
        ...
        if status == "selected":
            continue
        diagnostics.append(
            _unsupported_or_unknown_scalar_size_selected_tag_diagnostic(
                type_tag,
                status,
            )
        )
    ...
    valid_singletons: set[str] = set()
    for type_tag in selected_tags:
        if type_tag not in _SELECTED_SCALAR_SIZE_BYTES_TAGS:
            continue
        group = groups_by_name.get(type_tag)
        if group is None:
            diagnostics.append(_missing_scalar_size_singleton_diagnostic(type_tag))
            continue
        if group.members != (type_tag,):
            diagnostics.append(
                _inconsistent_scalar_size_singleton_diagnostic(group, type_tag)
            )
            continue
        valid_singletons.add(type_tag)
    ...
    rules = tuple(
        _scalar_size_rule_for_selected_tag(type_tag)
        for type_tag in selected_tags
        if type_tag in valid_singletons
    )
    return Result.ok(ScalarSizeBytesGenerationRuleSet(rules))


def build_scalar_size_bytes_generation_rule_set_from_catalog(
    catalog: Catalog,
) -> Result[ScalarSizeBytesGenerationRuleSet]:
    return build_scalar_size_bytes_generation_rule_set(catalog.type_groups)
...
def default_scalar_size_bytes_generation_rule_set() -> ScalarSizeBytesGenerationRuleSet:
    result = build_scalar_size_bytes_generation_rule_set(
        _default_scalar_size_type_groups()
    )
    if not result.is_ok:
        raise AssertionError(result.diagnostics)
    return result.unwrap()
...
def classify_scalar_size_bytes_generation_type_tag(
    type_tag: str,
) -> ScalarSizeBytesGenerationTagStatus:
    if type_tag in _SELECTED_SCALAR_SIZE_BYTES_TAGS:
        return "selected"
    if (
        bool(_CONCRETE_INTEGER_TAG_RE.fullmatch(type_tag))
        or bool(_FLOAT_TAG_RE.fullmatch(type_tag))
...
def _default_scalar_size_type_groups() -> tuple[TypeGroup, ...]:
    from pathlib import Path
    ...
    return tuple(
        TypeGroup(
            ...
            source_span=SourceSpan(
                SourceLocation(
                    Path("tsldata/detail/types.tsl"),
                    _TSDATA_TYPES_LINE_BY_SELECTED_SCALAR_SIZE_TAG[type_tag],
                    3,
                )
            ),
        )
        for type_tag in SELECTED_SCALAR_SIZE_BYTES_GENERATION_TYPE_TAGS
    )
...
def _normalize_scalar_size_tags(type_tags: tuple[str, ...]) -> tuple[str, ...]:
    return tuple(
        stable_sorted(
            tuple(dict.fromkeys(type_tags)),
            key=_scalar_size_tag_sort_key,
        )
    )


def _scalar_size_tag_sort_key(type_tag: str) -> tuple[int, int | str]:
    try:
        return (
            0,
            SELECTED_SCALAR_SIZE_BYTES_GENERATION_TYPE_TAGS.index(type_tag),
        )
    except ValueError:
        return (1, type_tag)


def _scalar_size_rule_for_selected_tag(
    type_tag: str,
) -> ScalarSizeBytesGenerationRule:
    for rule in SCALAR_SIZE_BYTES_GENERATION_RULES:
        if rule.type_tag == type_tag:
            return rule
    raise AssertionError(f"missing scalar size-bytes rule for {type_tag!r}")
...
def _unsupported_or_unknown_scalar_size_selected_tag_diagnostic(
    type_tag: str,
    status: ScalarSizeBytesGenerationTagStatus,
) -> Diagnostic:
    if status == "unknown":
        return Diagnostic.error(
            "TSL-DOMAIN-GEN-SIZE-RULE-TAG-UNKNOWN",
            "scalar size-bytes generation rule source received unknown selected "
            f"type tag {type_tag!r}",
        )
    return Diagnostic.error(
        "TSL-DOMAIN-GEN-SIZE-RULE-TAG-UNSUPPORTED",
        "scalar size-bytes generation rule source supports only selected "
        "singleton tags "
        f"{_quoted_join(SELECTED_SCALAR_SIZE_BYTES_GENERATION_TYPE_TAGS)}; "
        f"got {type_tag!r}",
    )
...
def _missing_scalar_size_singleton_diagnostic(type_tag: str) -> Diagnostic:
    return Diagnostic.error(
        "TSL-DOMAIN-GEN-SIZE-RULE-SINGLETON-MISSING",
        "scalar size-bytes generation rule source requires singleton type "
        f"group {type_tag!r}",
    )
...
def _inconsistent_scalar_size_singleton_diagnostic(
    group: TypeGroup,
    expected_type_tag: str,
) -> Diagnostic:
    return Diagnostic.error(
        "TSL-DOMAIN-GEN-SIZE-RULE-SINGLETON-INCONSISTENT",
        "scalar size-bytes generation rule source requires type group "
        f"{group.name!r} to contain exactly ({expected_type_tag!r},); got "
        f"{group.members!r}",
        location=group.source_span.location,
    )
