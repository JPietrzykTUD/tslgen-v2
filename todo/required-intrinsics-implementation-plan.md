...
| `_mm512_kunpackb` | `overlay_imask(a, b, 8)` is close, but it does not mask away the high bits of either input before concatenating the low bytes. | Add `concat_imask_low<HalfBits=8>(low, high) -> im`, explicitly taking the low `HalfBits` from each operand. |
