...
- Milestone 77 implements a behavior-preserving composable-lowering module
  boundary. It keeps the public lowering import surface stable while adding
  typed private modules for exact structural recognizers
  (`tslgen.lowering._exact_shapes`) and the exact array-body pipeline tail
  (`tslgen.lowering._pipeline`). The architecture rule for future lowering
...
- Post-M77 planning selects Milestone 78 as a behavior-preserving package
  decomposition of the accepted exact array-body / array-initialization
  lowering tail. It should keep `tslgen.lowering` and
  `tslgen.lowering.boundary` as stable public import surfaces while moving
  exact package-owned models/helpers/orchestration into private modules. The
  target is real maintainability improvement: `boundary.py` must shrink by at
...
- M83 implements generation-lowering stage output-contract ownership
  extraction. The accepted `GenerationLoweringStage` stage-name/output
  compatibility contract, the minimal accepted mini-TSIL value-model
  dependency, and `GenerationLoweringStage` now live in
  `tslgen.lowering._stage_contracts`, while `boundary.py` remains the public
  facade/coordinator. `boundary.py` now measures 4,807 physical lines. M83 is
...
- M84 implements exact array-body pipeline and source-adapter ownership
  extraction. It moved accepted M64-M76 exact array-body pipeline,
  source-adapter, and exact-array public lowerer ownership into private typed
  lowering modules while keeping `boundary.py` as the public facade for
  request/result models, selected-body public lowerers, lower-candidate
  orchestration, payload classification, and mini-TSIL lowering. This is
  behavior-preserving architecture work that shrank the facade to 1,898
...
- M88 is accepted as exact array-body structural package assembly. It adds
  ...
  aggregates accepted M64-M87 typed facts into one source-ordered structural
  package and appends `array_body_structural_package_assembly` after the M87
...
- M89 is accepted as exact array backend-deferred request inventory. It adds
  focused private `tslgen.lowering._array_body_backend_deferred_requests`
  ownership for one typed inventory over the accepted M88 package, with the
  accepted M72/M67 `value<backend>(uninit::array)` deferred backend-value
  boundary as the only supported member. It does not start Stage 9 backend
...
- M91 is accepted as exact array pipeline ownership consolidation. The
  architecture is behavior-preserving: `boundary.py` remains the public
  facade/projection, `_array_body_pipeline.py` is now orchestration at 591
  lines, `_array_body_pipeline_results.py` owns exact array result DTO/key
  behavior, and `_array_body_stage_assembly.py` owns stage/snapshot assembly.
...
- M92 is accepted as exact array lowering backend-handoff request ownership.
  The architecture uses focused private `_array_body_backend_handoff.py`
  ownership to consume accepted M90 completion packages and emit one typed
  request for later backend planning. The module keeps one-way imports and
...
- M93 is accepted as dual-source lowering operation package boundary
  ownership. The architecture uses focused private `_operation_package.py`
  ownership to package exactly accepted M86 mini-TSIL leaf return values and
  accepted M92 exact array backend-handoff requests as distinct Stage 8
  typed/provenance entries. It must not become a generic operation registry,
  ...
  coordinator.
- M94 is accepted as the operation-package maintainability split before
  package-family expansion. The architecture keeps `_operation_package.py` as
  a thin facade over focused private model, diagnostics, source-narrowing,
  mini-TSIL package-contract, and exact-array provenance modules. Those modules
...
- M98 is accepted as Stage 8 stage-assembly ownership extraction. The
  architecture now uses focused private `_lowering_stage_assembly.py`
  ownership for accepted `GenerationLoweringStage` construction and
  per-candidate operation-package -> completion-manifest ->
  completion-gap-inventory result assembly, while keeping `boundary.py` as the
  public facade and request/result model owner. The module stays narrow, keeps
  one-way imports, avoids importing `boundary.py` or the package facade, and
...
- M101 is accepted to prevent lowering IR taxonomy drift before adding more
  backend-result or body semantics. The architecture distinguishes
  semantic facts, requests, results, inventories, provenance values, rule
  inputs, and stage envelopes, and consolidates repeated provenance chains
  where that preserves accepted diagnostics and object identity. The accepted
  M101 code applies this only to the M99/M100 backend-translation
...
- M102 makes the taxonomy surface explicit as private category/protocol
  ...
  fixpoint machinery.
- M103 accepts a focused Stage 8 backend-translation boundary worklist
  inventory. Ownership belongs in focused private lowering modules rather than
  `boundary.py`, `_lowering_ir_contracts.py`, or the existing M99/M100 modules.
  The aggregate is an inventory/provenance view over accepted concrete M99
  request inventories and optional accepted concrete M100 results. It is not a
