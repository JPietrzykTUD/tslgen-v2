...
| Generation control | `if<generation>(...)`, `else<generation>`, `loop<unroll>(...)`, `loop<range>(...)`, `let<type>(...)`, and `var<...>(...)` | lowerable directive over generation context | Requires typed generation context and selected-branch behavior. Do not parse surrounding statements as a general language. |
...
| Arithmetic helper calls | `details::arith_add`, `details::arith_mul`, and `details::arith_rem` appear inside return expressions, assignments, and loops | helper substitution candidate | Mark for later substitution with backend-owned arithmetic operation spellings such as `+`, `*`, and `%` only when a focused milestone selects that island. Do not require parsing surrounding assignment, array access, or loop syntax. |
