...
| Intrinsic composition | `intrin_compose<add>(left, right)`, `intrin_compose<srli, suffix=value<backend>(...)>(...)`, and immediate metadata such as `immediate(2)=1` | backend-owned semantic operation | M166 records exact intrinsic request islands while preserving head/modifier and argument payload text opaque. M182 hands top-level `intrin_compose<...>` base/modifier fields to typed unresolved modifier facts and reuses M181 only for exact single `value<backend>(...)` modifier values. M195-M210 translate literal modifiers, type/current/symbol suffixes, prefixes, stream suffixes, `to_type_suffix`, literal immediates, selected `sImm` immediates, and selected indexed-vector generic immediates as typed backend modifier results. M213 assembles translated modifier results into typed invocation-shaped backend values while preserving argument payloads opaque and immediates as compile-time metadata. C++ non-type template rendering, Rust const generic rendering, Rust `core::arch::*` qualification, and backend call rendering remain deferred. Backend spellings come from backend translation rules, not hardwired generator tables. |
| Direct intrinsic calls | `intrin<_mm512_srl_epi32>(...)`, `intrin<svptrue_b8>()`, `intrin<svst1>(...)` | backend-owned operation | M166 records exact direct-intrinsic request islands; M213 assembles only already-literal direct intrinsic names into typed invocation values. Placeholder/template-like direct names, argument splitting, dependency inference, and rendering remain deferred. |
...
After M212, no next lowering milestone is selected from this inventory. The
active workflow selects backend intrinsic invocation assembly as the next
backend/output implementation slice. That milestone should consume accepted
intrinsic handoff requests, backend modifier translation results, and
source-owned opaque argument text rather than adding another lowering slice or
parsing arbitrary intrinsic arguments.
