...
| Generation/backend queries | `type<generation>(base::in)`, `value<generation>(vector::length)`, `type<backend>(vector::as_extension(scalar))`, and `value<backend>(intrin::suffix(...))` | typed query / translation request | Treat as lowerable query islands only with explicit typed context. M164 accepts exact `value<backend>(...)` islands as unresolved backend-owned query requests over source-owned text; payloads remain opaque. Rendering must consume translated values, not evaluate raw nested text. |
| Intrinsic composition | `intrin_compose<add>(left, right)`, `intrin_compose<srli, suffix=value<backend>(...)>(...)`, and immediate metadata such as `immediate(2)=1` | backend-owned semantic operation | M166 records exact intrinsic request islands while preserving head/modifier and argument payload text opaque. Backend spellings come from backend translation rules, not hardwired generator tables. |
...
| Cast/memory/I/O keyword calls | Examples include `cast<static>(...)`, `cast<reinterpret>(...)`, `cast<bitcast>(...)`, `cast<saturating>(...)`, `mem<copy>(...)`, `mem<alloc>(...)`, `mem<alloc_aligned>(...)`, `mem<free>(...)`, `io<write_base>(...)`, `io<write_bin>(...)`, `io<write>(...)`, and `io<endl>(...)` | source/backend-owned operation | M167 records exact request islands in source-owned text and contiguous raw body-token runs while preserving mode/operation and argument payload text opaque. It does not validate operation names, lower types inside payloads, split arguments, or render. |
