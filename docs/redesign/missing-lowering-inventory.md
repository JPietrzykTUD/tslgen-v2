...
- The clean restart M127-M156 line has accepted real TSIL payload intake,
  ...
  `value<generation>(...)`, and M160 exact selected-body
  `else if<generation>` branch-chain selection with an optional final
  `else<generation>` fallback. That path deliberately still does not render
  primitive-call expressions, parse raw arithmetic operators, parse broad TSIL
  expressions/statements, or support recursive generation control.
...
| Generation declaration/iteration lowering | `loop<unroll>(...)`, `loop<range>(...)`, `var<...>(...)`, non-type `let<...>(...)` | Generic/vector fallback bodies use TSIL directives for repeated statements, declarations, temporaries, and aliases. | M161 is selected next for an exact `loop<range>(...)` region fact with optional adjacent `loop<unroll>(...)` metadata over source-owned body tokens. `let<type>(...)` alias facts already feed the type environment. Do not execute loops, substitute loop variables, or parse all surrounding target-language statements. |
