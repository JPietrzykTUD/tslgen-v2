...
| Mask lane constants | `mask::lane::all_true`, `mask::lane::all_false` | M176 selects a typed backend/support-helper request boundary for exact mask lane constants. M177 should discover those exact request islands in source-owned text and preserve surrounding raw text/tokens. Backend helper rendering, source convention cleanup, and treating these constants as Python booleans remain out of scope. |
