...
- The clean restart M127-M156 line has accepted real TSIL payload intake,
  ...
  `else<generation>` fallback, and M161 exact `loop<range>(...)` region facts
  with optional adjacent `loop<unroll>(...)` metadata. That path deliberately
  still does not render primitive-call expressions, parse raw arithmetic
  operators, parse broad TSIL expressions/statements, execute loops, or
  support recursive generation control.
...
| Generation declaration/iteration lowering | `loop<unroll>(...)`, `loop<range>(...)`, `var<...>(...)`, non-type `let<...>(...)` | Generic/vector fallback bodies use TSIL directives for repeated statements, declarations, temporaries, and aliases. | M161 accepts an exact `loop<range>(...)` region fact with optional adjacent `loop<unroll>(...)` metadata over source-owned body tokens. `let<type>(...)` alias facts already feed the type environment. Loop execution/unrolling, loop-variable substitution, declarations, and broad surrounding target-language statement parsing remain deferred. |
