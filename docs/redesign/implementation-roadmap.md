...
Accepted. The M126 execution-review loop returned `Accept With Follow-Ups`
after one write-capable executor and read-only architecture, boundary,
documentation, and validation audits. The follow-ups were finalization-only:
mark M126 accepted, update workflow state, and create the integrated M127
execution prompt.
...
- Add exact source recognition for binary scalar implementation body lines
  ...
  one-primitive / one-or-more-implementation document shape.
- Preserve the accepted synthetic `body <operation>(left, right)` form, all
  accepted primitive header shapes, selected-implementation lowering behavior,
  operation descriptors, scalar type descriptors, compatibility rules, and the
  `clean_restart_bootstrap_core` semantic-origin contract.
- Promote the exact TSIL emit-return body into the existing typed binary
  operation body value consumed by lowering; do not introduce a new lowering
  IR category, request/result family, registry, dispatcher, or raw-text
  rewrite path.
- Prove selected TSIL emit-return bodies drive lowering and generated C++/Rust
  artifacts for at least one representative binary primitive.
- Prove unselected exact TSIL emit-return bodies are not lowered by including
  a multi-implementation document where an unselected exact TSIL body would be
  a lowering mismatch if selected, while the selected implementation still
  generates successfully.
- Add negative tests for selected mismatched TSIL emit-return operation bodies
  and malformed nearby TSIL forms, producing structured diagnostics rather
  than source repair, renderer inference, or silent fallback.
...
- Lowering consumes a typed binary operation body value and does not emit from
  raw TSIL text.
- Selected mismatched TSIL emit-return bodies produce the existing structured
  lowering mismatch diagnostic.
- Malformed nearby TSIL forms are rejected with structured diagnostics and are
  not repaired or normalized.
...
- Reviewers should require M126 to remain an exact selected-body lowering
...
- Reviewers should reject nested helper parsing, primitive-call lowering,
  ...
  repair.
- Reviewers should require TSIL emit-return recognition to produce typed body
  values before backend emission; generated output must not be rendered by
  rescanning raw TSIL text.

Review result:

- Parser-owned exact recognition now accepts only the binary scalar body line
  `tsil "emit_return(<operation>(left, right));"` under the accepted binary
  primitive signature.
- The accepted TSIL spelling is immediately promoted into existing typed
  parsed body data and then existing `BinaryOperationBody` catalog data before
  selection, lowering, or backend emission.
- Selected TSIL emit-return bodies drive the same generated C++/Rust artifacts
  as the accepted synthetic body form.
- Unselected exact TSIL emit-return bodies do not create lowering diagnostics.
- Selected mismatched TSIL emit-return bodies and malformed nearby TSIL forms
  produce structured diagnostics rather than source repair or renderer
  inference.
- Existing synthetic body behavior, M125 multi-implementation behavior, M124
  multi-source behavior, `clean_restart_bootstrap_core` semantic-origin
  records, backend-owned spellings, source-body integrity, deterministic
  artifact ordering, and representative artifact bytes remained stable.
...
- Add exact source recognition for unary scalar implementation body lines
  shaped as `tsil "emit_return(<operation>(value));"` under the existing
  unary primitive header form `prim<v:=(v)> name(value):`.
- Preserve the accepted synthetic `body <operation>(value)` form, the accepted
  M126 binary TSIL emit-return form, all accepted primitive header shapes,
  selected-implementation lowering behavior, operation descriptors, scalar
  type descriptors, compatibility rules, and the
  `clean_restart_bootstrap_core` semantic-origin contract.
- Promote the exact unary TSIL emit-return body into the existing typed unary
  operation body value consumed by lowering; do not introduce a new lowering
  IR category, request/result family, registry, dispatcher, or raw-text
  rewrite path.
- Prove selected unary TSIL emit-return bodies drive lowering and generated
  C++/Rust artifacts for at least one representative unary primitive.
- Prove unselected exact unary TSIL emit-return bodies are not lowered by
  including a multi-implementation document where an unselected exact unary
  TSIL body would be a lowering mismatch if selected, while the selected
  implementation still generates successfully.
- Add negative tests for selected mismatched unary TSIL emit-return operation
  bodies and malformed nearby unary TSIL forms, producing structured
  diagnostics rather than source repair, renderer inference, or silent
  fallback.
- Preserve M126 binary TSIL behavior, M125 multi-implementation behavior, M124
...
- A selected unary scalar implementation may use either the accepted synthetic
  `body <operation>(value)` line or the exact TSIL emit-return line
  `tsil "emit_return(<operation>(value));"`.
- Lowering consumes a typed unary operation body value and does not emit from
  raw TSIL text.
- Selected mismatched unary TSIL emit-return bodies produce the existing
  structured lowering mismatch diagnostic.
- Malformed nearby unary TSIL forms are rejected with structured diagnostics
  and are not repaired or normalized.
- Existing M107-M126 behavior and representative artifact bytes remain stable.
...
- Reviewers should require M127 to remain an exact selected-body lowering
  slice, not a broad TSIL parser or compiler milestone.
- Reviewers should reject nested helper parsing, primitive-call lowering,
  intrinsics, casts, generation-time helpers, branch pruning, backend manifest
  loading, renderer-side semantic inference, target discovery, or source
...
Accepted. The M129 execution-review loop returned `Accept` after one
write-capable executor and read-only architecture, boundary, documentation,
and validation audits. M129 kept the task focused on lowering and added the
next corpus-observed exact scalar comparison TSIL emit-return body spelling:
...
`tsldata/primitives/comparison/fundamental.tsl:192`; this evidence did not
...
Validation completed with the required M129 profile. `git diff --check`
returned exit 0 with no output. The public API import command returned exit 0
with no output. The py-compile command returned exit 0 with no output. The
targeted clean-package pytest command returned exit 0 with
...
- Preserve the accepted synthetic `body <operation>(left, right)` comparison
  form, the accepted M126 binary TSIL emit-return form, the accepted M127
  unary TSIL emit-return form, the accepted M128 equality TSIL emit-return
  form, the accepted M129 inequality TSIL emit-return form, all accepted
...
Accepted. The M131 execution-review loop returned `Accept With Follow-Ups`
after one write-capable executor, one focused cache-cleanliness revision
executor, and read-only architecture, boundary, documentation, and validation
audits. Follow-ups were finalization-only and were handled by updating
workflow state, roadmap state, and ADR wording before rerunning required
validation. M131 kept the task focused on lowering and added the next
...
`tsldata/primitives/bitwise/bit_ops.tsl:462`; this evidence did not become a
...
Validation completed with the required M131 profile. `git diff --check`
returned exit 0 with no output. The public API import command returned exit 0
with no output. The py-compile command returned exit 0 with no output. The
targeted clean-package pytest command returned exit 0 with
`142 passed in 7.71s`.
...
- Preserve M131 exact binary operator TSIL behavior, M130 ordered comparison
  ...
  selection, compatibility diagnostics, and deterministic artifact ordering.

Out of scope:

- Broad `tsldata` syntax/layout parsing, nested implementation maps, multiple
  primitive blocks in one document, attributes, tests, descriptions, `requires`
  clauses, type groups, extension fallback, dependency closure, or target
  discovery.
...
- Selected binary scalar implementations may declare non-`left/right`
  parameter names and still lower when the accepted body form uses those names
...
- Reviewers should require M132 to remain a binary declared-parameter
  lowering slice, not broad `tsldata` parsing, source repair, aliasing, or TSIL
  expression parsing.
- Reviewers should reject broadening comparison/unary parameters, adding new
  operator spellings, primitive aliasing, target discovery, backend manifest
