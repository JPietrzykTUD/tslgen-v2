...
| TSIL semantic/lowering parity: legacy TSIL grammar and compiler handle calls, intrinsics, variables, loops, generation-time conditions, casts, type/value metadata, and cleanup passes. | `frozen/tsl-gen/tsl_gen/tsil.lark`, `frozen/tsl-gen/tsl_gen/tsil_engine/compiler.py`, `frozen/tsl-gen/tsl_gen/tsil_engine/passes/*.py`, `tsldata/primitives/**.tsl` | `required-now` for the next selected intrinsic-compose form; `required-later` for full semantic TSIL | Typed-opaque lowering plus direct parameter-add mini-lowering | No typed model for `intrin_compose`, semantic primitive calls, loops, variables, type/value queries, or `if<generation>` | M38 for intrinsic compose; later milestones for calls/loops/generation-time conditions | Unit lowering fixtures, unsupported-form diagnostics, deterministic lowered IR, renderer consumes typed lowering only |
