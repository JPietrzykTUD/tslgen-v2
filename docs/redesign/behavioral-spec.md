...
vector-alignment request resolution. It should consume accepted M67/M68/M69/M70
typed request/result/pipeline values and explicit typed vector-alignment
metadata supplied before lowering evaluation, then produce a typed
vector-alignment resolution value/stage after the vector-length stage. It must
preserve M70 behavior, leave backend uninit unresolved, and must not infer
...
maps, backend vector-alignment spellings, or renderers.
