...
The selected post-M50 plan is Milestone 51, a generation-time semantic lowering
slice that accepts only the same M48 signedness predicate branch form with
plain `else`. M51 must consume typed M43
`GenerationTypeRef(kind="base.in")` values, preserve M42/M48 branch provenance
