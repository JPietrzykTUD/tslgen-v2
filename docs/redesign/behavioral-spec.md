...
Milestone 64 is selected as the exact array-body envelope slot assembly slice.
...
| TSIL semantic/lowering parity: legacy TSIL grammar and compiler handle calls, intrinsics, variables, loops, generation-time conditions, casts, type/value metadata, and cleanup passes. | `frozen/tsl-gen/tsl_gen/tsil.lark`, `frozen/tsl-gen/tsl_gen/tsil_engine/compiler.py`, `frozen/tsl-gen/tsl_gen/tsil_engine/passes/*.py`, `tsldata/primitives/**.tsl` | `required-now` for selected intrinsic-compose, base-type, signedness branch-pruning, scalar size-byte value, selected scalar bit-width arithmetic, selected size-byte equality predicate, exact size-byte branch-chain pruning, opaque selected-body handoff, selected assignment-form recognition, selected assignment/direct-intrinsic body IR, selected body-envelope IR, and selected exact array-body slot-envelope slices; `required-later` for full semantic TSIL | Typed-opaque lowering, direct parameter-add mini-lowering, M38 selected intrinsic-compose helper lowering, M41 generation-time lowering contract, M42 aligned primitive-attribute branch pruning, M43 base type query values, M48 signedness branch pruning, M51 exact plain-`else` signedness branch support, M52 concrete integer type/signedness expansion, M53 typed rule-source boundary, M54 catalog-derived rule wiring, M55 scalar size-byte value query, accepted M56 exact size-bytes-times-eight value arithmetic, accepted M57 exact size-byte equality predicate lowering, accepted M58 staged lowering contract, accepted M59 exact size-byte branch-chain pruning, accepted M60 opaque selected-body handoff, accepted M61 selected assignment-form recognition, accepted M62 unresolved selected assignment/direct-intrinsic body IR, accepted M63 backend-neutral selected-body envelope IR, and selected M64 exact array-body envelope slot assembly | No semantic primitive calls, loops, variables, broad type/value queries, broad direct intrinsics, broad `intrin_compose` metadata, generalized plain `else` generation branches, vector/register metadata, general generation-value comparisons beyond the accepted M57 predicates, branch-chain pruning beyond the accepted M59 exact chain, broad generation-value arithmetic beyond the accepted M56 expression, or branch body parity beyond selected M60 handoff, M61 form-recognition metadata, accepted M62 unresolved body IR, accepted M63 singleton envelope IR, and selected M64 exact opaque slot envelope | M38 selected helper lowering, M40 translation/boundary correction, M41-M43 accepted lowering slices, M48 signedness pruning, M51 plain-`else` signedness branch slice, M52 concrete integer type/signedness slice, M53 rule-source boundary, M54 catalog-derived rule wiring, M55 scalar size-byte value query, accepted M56 value arithmetic, accepted M57 predicate lowering, accepted M58 stage boundary, accepted M59 branch-chain pruning, accepted M60 handoff, M61 assignment-form recognition, accepted M62 assignment/direct-intrinsic body IR, accepted M63 selected-body envelope IR, and selected M64 exact array-body slot envelope | Unit lowering fixtures, unsupported-form diagnostics, deterministic lowered IR/stage outputs, selected-branch-only diagnostics, generation-value diagnostics, typed handoff/form-recognition/body-IR/envelope/slot-envelope diagnostics, translation metadata tests, backend rejection of unresolved generation-time helpers, renderer non-evaluation |
