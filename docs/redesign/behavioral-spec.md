...
The selected post-M47 plan is Milestone 48, a generation-time semantic lowering
slice for signedness type-predicate branch pruning. M48 must evaluate only
`value<generation>(type::is_signed(type<generation>(base::in)))` from typed
M43 `GenerationTypeRef(kind="base.in")` values, prune exact
`if<generation> ... else<generation>` branches with M42-style selected-branch
provenance, and keep unselected branch helpers from producing diagnostics.
It must not add backend translation, backend rendering, generated output,
