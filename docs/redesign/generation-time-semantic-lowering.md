...
| mask lane constants | implemented by M177 request boundary | M176 decided exact mask lane constants are typed backend/support-helper requests, not normal generation values; M177 discovers the exact request islands without rendering helper text. |
...
| `value<backend>(uninit::array)` | Exact remaining M67 array-initialization request in `tsldata/primitives/load_store/array.tsl:105`; backend output-map evidence in `tsldata/detail/lang/translate_cpp.tsl` | Preserve the backend-uninit helper as typed deferred backend-value request state so the exact first-slot helper set can become one typed handoff, then resolve the accepted M99 exact-array request to a typed C++ translation-result boundary. | Accepted M67 request record, accepted M68/M70/M71 resolution chain, accepted M99 request inventory record, candidate/slot provenance, and explicit typed C++ rule input. | Narrow typed backend-uninit request/result boundary for the exact array case. | C++ `value_array_uninit` rule supplied as typed metadata; no backend map, catalog, manifest, renderer, or `tsldata` read during lowering. | M72 implemented helper-set completion; M99 inventories the exact request; M100 accepts exact C++ translation-result state only. | M100 does not translate Rust, render output, create Stage 9 plans, lower declarations/body semantics, or introduce generic backend-value evaluation. | M100 coverage includes positive C++ exact-array result, missing/duplicate/conflicting typed rule diagnostics, unsupported backend/Rust diagnostics including mixed valid+unsupported rules, object identity back through M99/M92/M72/M67, determinism, source/container diagnostics, no backend map/catalog/`tsldata` reads, no renderer/output, and no source repair. |
...
| `value<generation>(mask::lane::all_true)` and `value<generation>(mask::lane::all_false)` | `tsldata/primitives/bitwise/bit_ops.tsl:608`, `:1559`; `tsldata/primitives/comparison/fundamental.tsl:57`, `:216`; `tsldata/primitives/conversion/mask_specific.tsl:192` | Preserve all-true/all-false mask lane constants as backend/support-helper needs. | Source polarity, source provenance, and later backend support-helper rules. | Typed backend/support-helper request, not `GenerationValue[int|bool]`. | Backend/helper translation rules later; no helper text in lowering. | M177 accepted request discovery. | Exact request islands are discovered in source-owned text and selected body raw-token text while surrounding raw text/tokens stay opaque; backend rendering remains later. | Exact all_true/all_false request tests, malformed/unknown diagnostics, no Python bool/int or backend string values, corpus context tests. |
...
M89 is accepted as exact array backend-deferred request inventory. It consumes
the accepted M88 structural package and inventories only the accepted M72/M67
`value<backend>(uninit::array)` deferred backend-value boundary as typed
lowering data. This is not backend-uninit resolution: the inventory preserves
`deferred_backend_value` policy, source/provenance identity, and typed request
fields without reading backend maps, querying backend catalogs, creating
...
Accepted M96 manifest diagnostics stay on the Stage 8 lowering side. The
manifest reports `TSL-LOWER-COMPLETION-MANIFEST-SOURCE-UNSUPPORTED`,
...
`TSL-LOWER-COMPLETION-MANIFEST-DEPENDENCY-PROVENANCE-MISMATCH`, and
`TSL-LOWER-COMPLETION-MANIFEST-SOURCE-AMBIGUOUS` for unsupported or empty
package sources, duplicate package keys, malformed package entries, mixed
candidate context, wrong stage/order, source-location/provenance mismatches,
ambiguous package containers, and unresolved dependency provenance mismatches.
...
- Accepted M96 summarizes accepted M86/M92/M95 package facts and accepted
  unresolved backend-handoff dependencies as a deterministic lowering-side
  manifest only, preserving package and dependency references by object
  identity. Backend readiness decisions, backend value resolution, operation
...
- M97 summarizes accepted M96 manifest facts as a deterministic lowering-side
  gap inventory only. The first supported gap category is accepted unresolved
  backend-handoff dependency records, and manifests without such records
...
- M98 is accepted as behavior-preserving Stage 8 stage-assembly ownership
  ...
  preserving M57-M97 semantics, diagnostics, stage order, keys, and
...
- M100 is accepted as a typed C++ exact-array backend-uninit translation-result
  boundary over accepted M99 `exact_array_backend_value_uninit_array` records.
  M100 may consume explicit typed C++ `value_array_uninit` rule values, but it
  must not read backend maps/catalogs/manifests or `tsldata/detail/lang` during
  lowering. Rust, generic backend helper evaluation, selected-body
  direct-intrinsic/SVE semantics, renderer-ready IR, Stage 9 planning,
  rendering, generated output, and broad body semantics remain deferred.
