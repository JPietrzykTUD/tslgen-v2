...
The M170 execution-review loop returned `Accept With Follow-Ups` after one
write-capable executor and read-only architecture, boundary, evidence, test,
documentation, and validation audits. M170 extracted selected specialization
binding validation/resolution into a focused lowering helper shared by
type-query and selector-payload lowering, reducing `type_queries.py` below the
module-size guardrail area. It made explicit M169 selected binding facts
visible to exact bare primitive-call selector payload entries: return-type
base bindings lower to scalar type values, return-type extension bindings
lower to extension operands, and vector/type bindings lower to `CurrentVector`.

M170 preserved raw `SelectorSymbol` behavior for unbound arbitrary selector
names, diagnosed declared extension binding names without selected facts
instead of falling through to raw extension/symbol behavior, and preserved
M144-M151 primitive-call behavior, M168 generation-expression behavior, and
M168.5/M169 selected binding behavior. It did not parse the full selector
...
The M172 execution-review loop returned `Accept With Follow-Ups` after one
write-capable executor and read-only architecture, boundary, evidence, test,
documentation, and validation audits. M172 extended primitive-call target
matching so the existing concrete-vector extraction helper accepts already
lowered `LoweredVectorTransformType` selector values when their extension is
concrete and their base type reduces to a concrete scalar `TypeTag`.

M172 preserved existing `CurrentVector`, `LoweredBackendTypeReference`, and
`LoweredVectorAsExtensionType` matching behavior, preserved M171
selected-return-binding provenance and target decoration for the two-entry
vector-plus-return-binding shape, and kept alias spellings source-local rather
than semantic. Raw selector symbols, literals, known extension operands in
vector position, unresolved specialization symbols, and mask/member aliases
that do not expose a concrete scalar type tag remain diagnostics. M172 did not
...
The M179 execution-review loop returned `Accept` after one write-capable
executor and read-only architecture, boundary, test, documentation, and
validation audits. M179 added exact `type<backend>(...)` backend type query
request-island discovery over source-owned text and contiguous raw body-token
runs. Discovered islands preserve payload text, complete source island text,
payload source, island source, and surrounding opaque text/tokens.

M179 keeps raw discovered islands distinct from already-lowered
`BackendTypeSpellingRequest` values. It preserved the existing selected-context
`lower_backend_type_query(...)` semantic boundary and M164
`value<backend>(...)` behavior, and added no backend type spelling
...
The M136 execution-review loop returned `Accept` after one write-capable
executor and read-only architecture, boundary, documentation, and validation
audits. M136 added ordered source-owned argument-list representation for
recognized TSIL `call<primitive=...>(...)` tokens while preserving the
original opaque `PrimitiveCall.payload` string. Each `PrimitiveCallArgument`
keeps raw argument text and source location. The splitter only recognizes
top-level commas and respects nested parentheses and square brackets. Zero
argument calls produce an empty tuple.

M136 populates argument lists for standalone M132 call tokens and for M134
`emit_return(...)` payload call tokens. It preserves M133/M134 exact
`call<primitive=add>(left, right)` lowering by requiring both the exact opaque
payload and exact structured argument texts. It did not resolve named
primitive references against the catalog, expand `@self`, interpret selector
specialization or attrs payloads, resolve argument identifiers, parse array
access, assignment, operators, helpers, casts, or nested call semantics,
recursively lower arguments, add dependency closure, add backend call
rendering, repair source, or introduce runtime `tsldata`, `frozen`, or
`tslgenold` dependencies.
...
The M146 execution-review loop returned `Accept With Follow-Ups` after one
write-capable executor and read-only architecture, boundary, evidence,
documentation, and validation audits. M146 added a typed primitive-call
argument-binding boundary that consumes a recognized `PrimitiveCall` and an
M145 target match, then binds ordered raw `PrimitiveCallArgument` objects
positionally to the matched primitive's formal parameter names.

M146 preserves the original primitive call, target match, raw argument text,
argument source locations, and call source provenance. It accepts arbitrary
raw argument text, including swapped identifiers, duplicate identifiers,
array-looking text, helper/operator-looking text, and nested
`call<primitive=...>(...)` text when the arity matches. It reports
`TSL-LOWER-PRIMITIVE-CALL-ARGUMENT-COUNT-MISMATCH` at the primitive-call
source when the argument count differs from the matched primitive's parameter
count.

M146 did not parse argument expressions, recursively lower nested calls,
...
or backend type text, repair source, or introduce runtime `tsldata`,
`frozen`, or `tslgenold` dependencies.

M146 review verdicts were: architecture `Accept With Follow-Ups`, boundary
`Accept`, evidence `Accept`, documentation `Accept`, and validation `Accept`.
The recorded nonblocking M146 follow-up is to tighten the public lowerer
argument-binding API later: either remove redundant `selected`,
`selector_payload`, and `catalog` parameters or validate their relationship to
the supplied target match before broader call-lowering work builds on it.

M146 validation completed with:
...
The M155 execution-review loop returned `Accept` after focused test and
documentation revisions. M155 added a small typed generation-value result
boundary and isolated selected-context `value<generation>(...)` lowering for
`vector::length`, `vector::alignment`,
`type::size_bytes(TYPE_EXPR)`, `type::is_signed(TYPE_EXPR)`,
`type::is_same(TYPE_EXPR, TYPE_EXPR)`, and
`primitive::attribute(KEY)`.

M155 reuses the accepted type syntax/type lowering path for `TYPE_EXPR`
arguments instead of exact raw nested string matching. It evaluates only
supported scalar lowered type values and selected concrete boolean primitive
attributes, and it reports deterministic diagnostics for malformed queries,
unsupported value families, unsupported lowered type values such as
...
M155 did not implement generation-control branch pruning, loop execution,
declaration lowering, selector-attribute substitution, mask constants, generic
lengths, backend rendering, raw text replacement, source repair, broad
expression parsing, registries, dispatchers, worklists, runtime `tsldata`,
`frozen`, or `tslgenold` dependencies.

M155 review verdicts were: architecture `Accept`, boundary `Accept`, evidence
`Accept With Follow-Up`, test `Accept` after focused re-review, documentation
`Accept` after focused domain-model re-review, and validation `Accept`. The
evidence follow-up was the process note to include the new
`generation_values.py` module in the final changed file set; it is not a
product follow-up.

M155 validation completed with:
...
The M156 execution-review loop returned `Accept` after focused architecture,
boundary, test, and documentation revisions. M156 added a focused typed
generation-control branch-region boundary over existing source-owned body
tokens. It lowers only exact full-body
`if<generation>(VALUE_QUERY) { ... } else<generation> { ... }` regions whose
condition lowers through M155 to a boolean primitive-attribute, scalar
signedness, or scalar type-sameness value.

M156 records the lowered condition plus selected and unselected branch token
slices with source provenance. Branch body tokens are preserved exactly; raw
branch text containing target-language `else { ... }` or comment-like
`else if<generation>` remains branch content, not structural generation-control
syntax. Structural `else if<generation>` branch chains and plain `else` forms
are deterministic unsupported diagnostics.

M156 did not implement selected-branch body lowering, body rendering, backend
rendering, loops, declarations, raw expression parsing, arithmetic/comparison
folding around generation values, selector-attribute substitution, generic
lengths, dependency scheduling, source repair, runtime `tsldata`, `frozen`, or
`tslgenold` dependencies, registries, dispatchers, worklists, or fixpoint
machinery.

M156 review verdicts were: architecture `Accept` after focused re-review,
boundary `Accept` after focused re-review, evidence
`Accept With Follow-Ups`, test `Accept` after focused re-review,
documentation `Accept` after focused re-review, and validation `Accept` before
the focused revision. The evidence follow-up to record representative
plain-else evidence was addressed during finalization, and final validation was
rerun by the orchestrator after the accepted revision.

M156 validation completed with:
...
The M111 execution-review loop returned `Accept With Follow-Ups`. M111
broadened the tiny clean lowering path from a one-off `add` operation check
into a small lowering-owned binary operation descriptor table for `add`,
`sub`, and `mul`. `LoweredFunction` now carries a backend-neutral binary
operation expression with an operation descriptor alongside the M110 scalar
type descriptor. C++ and Rust operator spellings remain backend-owned in their
emitters. The parser/catalog still preserve the exact tiny scalar binary
source shape while allowing identifier-like primitive/body operation names,
and syntactically valid but unsupported operation ids fail in lowering with
`TSL-LOWER-UNSUPPORTED-OPERATION`. A supported primitive whose body uses a
...
did not import from `tslgenold/` or `frozen/`, add CLI work, writer changes,
vector/SIMD semantics, broad TSIL/expression parsing,
backend-manifest/operation-map reads, old operation/lowering migration,
...
The M113 execution-review loop returned `Accept`. M113 made the lowered
function signature explicit for the tiny clean lowering path. `LoweredFunction`
now carries a backend-neutral `LoweredFunctionSignature` paired with the
accepted M112 `LoweredFunctionBody`. The signature contains only the
deterministic function name, source primitive name, ordered parameters, and
scalar type descriptor. C++ and Rust emitters render from the explicit
signature/body pair while keeping language syntax, type spelling, operator
spelling, logical paths, and metadata backend-owned. Existing artifact bytes,
logical paths, ordering, descriptor tables, body values, lowering diagnostics,
and digests remain stable. M113 did not import from `tslgenold/` or `frozen/`,
add parser/source syntax changes, source repair, `emit_return(...)`
recognition, extra arities/parameter names, function overloading policy,
namespaces/modules/packages, include planning, artifact layout changes, CLI
work, writer changes, generated test execution, CMake/Cargo scaffolding,
...
The M115 execution-review loop returned `Accept With Follow-Ups` after
read-only layout/boundary, architecture, documentation, and validation audits.
M115 added `div` to the tiny clean lowering-owned binary operation descriptor
table and backend-owned C++/Rust operator spelling tables. It preserved the
M110 scalar descriptors, M112 explicit return body, M113 explicit signature,
M114 lowering stage-output boundary, existing `add`/`sub`/`mul` outputs,
diagnostics, logical paths, ordering, and digests. M115 did not add
...
backend manifests, dependency closure, old operation/lowering migration,
registries, dispatchers, plugin systems, hidden backfeeds, fixpoint
mechanisms, or a broad operation/type framework.

The M116 execution-review loop returned `Accept` after read-only
layout/boundary, architecture, documentation, and validation audits. M116
added integer-only `mod` to the tiny clean lowering-owned binary operation
descriptor table, added a focused lowering-owned operation/type compatibility
rule boundary, and added backend-owned C++/Rust `%` spellings for accepted
lowered `mod` functions. Floating `mod` over accepted `f32`/`f64` scalar
descriptors now fails in lowering with
`TSL-LOWER-UNSUPPORTED-OPERATION-TYPE`. M116 preserved accepted
`add`/`sub`/`mul`/`div` behavior, M110 scalar descriptors, M112 explicit
return body, M113 explicit signature, M114 stage-output behavior, diagnostics,
logical paths, ordering, and existing artifact bytes/digests. M116 did not add
parser/source syntax changes, source repair, floating modulo semantics,
runtime remainder policy, divide-by-zero diagnostics, integer overflow policy,
...
The M118 execution-review loop returned `Accept` after read-only
layout/boundary, architecture, documentation, and validation audits. M118
added the exact unary `bit_not(value)` source/lowering shape, modeled accepted
unary source bodies as typed `UnaryOperationBody` catalog values, added a
backend-neutral unary operation descriptor and lowered unary expression, and
kept C++/Rust unary spellings backend-owned (`~value` for C++ and `!value` for
Rust). `bit_not` lowers only for accepted integer scalar descriptors, while
accepted floating scalar descriptors fail in lowering with
...
body values, M113 signatures, M114 stage-output behavior, M116/M117
compatibility behavior, diagnostics, logical paths, ordering, and existing
artifact bytes/digests. M118 did not add broad TSIL parsing, arbitrary arity,
multiple statements, nested expressions, calls, variables, source repair,
logical boolean semantics, masks, shifts, rotates, bit-width promotion,
signedness runtime policy, constant folding, algebraic simplification,
vector/SIMD semantics, backend manifests, dependency closure, old
operation/lowering migration, registries, dispatchers, plugin systems, hidden
backfeeds, fixpoint mechanisms, or a broad expression/type framework.

The M119 execution-review loop returned `Accept With Follow-Ups` after one
write-capable executor and read-only layout/boundary, architecture,
documentation, and validation audits. The only documentation follow-ups were
the normal orchestrator state, roadmap, and next-prompt updates completed
during finalization. M119 added backend-neutral unary `neg` after `bit_not` in
the clean unary operation descriptor table, gated `neg` to accepted `si32`,
`f32`, and `f64` scalar descriptors, rejected `ui32` with
`TSL-LOWER-UNSUPPORTED-OPERATION-TYPE`, and kept C++/Rust `-` spelling in the
backend layers. It preserved accepted binary behavior, M118 `bit_not`
behavior, diagnostics, logical paths, ordering, and existing artifact bytes.
M119 did not add source syntax beyond the accepted unary form, source repair,
runtime overflow/wrapping or unsigned-negation policy, floating special-value
...
The M121 execution-review loop returned `Accept With Follow-Ups` after one
write-capable executor and read-only layout/boundary, architecture,
documentation, and validation audits. M121 added the first exact scalar
comparison result lowering shape for `equal(left, right)` over the documented
`m:=(v,v)` source form. It models comparison bodies as typed catalog data,
lowers accepted `si32`, `ui32`, `f32`, and `f64` equality comparisons into a
backend-neutral comparison expression with an explicit scalar-comparison result
boundary, and keeps C++/Rust `bool` result spelling plus `==` operator spelling
owned by the backend layers. It preserved accepted binary and unary behavior,
M110 scalar descriptors, M112 body values, M113 signatures, M114 stage-output
behavior, M116-M120 compatibility behavior, diagnostics, logical paths,
artifact ordering, and existing artifact bytes. M121 did not add broad mask
modeling, vector/SIMD comparison results, comparison families beyond equality,
...
Post-M122 planning selected
`Milestone 123: Tiny Clean Bootstrap Operation Semantics Contract Slice`.
Internal planning review accepted the selected lowering-focused plan after a
focused workflow handoff revision. The selected plan responds to the
product-owner concern that operation facts observed in `tsldata/*` must not
silently leak into product generator code by making accepted M111-M122 scalar
operation descriptors and operation/type compatibility rules explicit
bootstrap-core lowering semantics. It must not load operation semantics from
...
Completed M111 execution-review-loop prompt:
...
Completed M113 execution-review-loop prompt:
...
Completed M115 execution-review-loop prompt:

```text
docs/agent/runs/m115-execution-review-loop-prompt.md
```

Completed M116 execution-review-loop prompt:
...
Completed M118 execution-review-loop prompt:

```text
docs/agent/runs/m118-execution-review-loop-prompt.md
```

Completed M119 execution-review-loop prompt:
...
Completed M121 execution-review-loop prompt:
...
Completed post-M122 lowering planning-plus-review prompt:
...
- M111 is limited to a tiny clean binary-operation lowering table over the
  ...
  expression/type framework.
- M112 is limited to adding an explicit lowered function-body/return-statement
  boundary over the accepted M111 binary operation expression; it must not add
...
- M114 is limited to adding an explicit lowering stage-output boundary for
  ordered lowered function sets plus diagnostics before backend emission; it
...
- M115 is limited to adding `div` to the existing tiny clean binary-operation
  ...
  framework.
- M116 is limited to adding integer-only `mod` to the existing tiny clean
  binary-operation lowering path plus a small lowering-owned operation/type
  compatibility boundary over accepted scalar descriptors; it must not add
  parser/source syntax changes, source repair, floating modulo semantics,
  runtime remainder policy, divide-by-zero diagnostics, vector/SIMD semantics,
...
- M117 is limited to adding integer-only `bit_and`, `bit_or`, and `bit_xor`
  through the existing tiny clean binary-operation descriptor, M116
  operation/type compatibility, and M114 stage-output paths; it must not add
  parser/source syntax changes, source repair, logical boolean semantics,
...
- M119 is limited to adding exact unary `neg(value)` through the accepted M118
  one-parameter source/lowering shape, accepting signed integer and floating
  scalar descriptors while rejecting unsigned scalar descriptors with the
  existing operation/type compatibility diagnostic. It must not add broad TSIL
  parsing, source repair, runtime overflow/wrapping policy, algebraic
  simplification, vector/SIMD semantics, backend manifests, old operation
  migration, registries, dispatchers, hidden backfeeds, fixpoint behavior, or
  a broad expression/type framework.
- M120 is limited to adding exact integer `shift_left(left, right)` and
  `shift_right(left, right)` through the accepted binary source/lowering
  shape, with backend-neutral descriptors, lowering-owned integer type gates,
  and backend-owned C++/Rust shift spellings. It must not add new source
  syntax, shift-count range or width policy, arithmetic-vs-logical shift
...
- M122 is limited to broadening the accepted M121 same-shape scalar comparison
  path from `equal` to `nequal`, `less_than`, `greater_than`,
  `less_than_or_equal`, and `greater_than_or_equal` using backend-neutral
  comparison descriptors and backend-owned C++/Rust result/operator spellings.
  It must not add new compare source syntax, mask/vector/SIMD compare results,
  compare-zero/range/special forms, runtime NaN or signed-ordering policy
  beyond backend operator emission, source repair, backend manifests,
  registries, dispatchers, hidden backfeeds, fixpoint behavior, or a broad
  expression/type framework.
- M123 is limited to making the accepted M111-M122 scalar operation
  descriptors and operation/type compatibility rules explicitly identify their
  semantic origin as accepted bootstrap-core lowering semantics. It must not
  add new operations, source syntax, scalar types, backend spellings, backend
...
- M110 documentation follow-up: consider adding a small cross-reference from
  ...
  accepted surface.
- M111 documentation follow-up: consider adding one sentence to the M111
...
- M121 validation follow-up addressed by M122: comparison-family tests now
  ...
  `equal(value, right)`.
- Product-owner lowering follow-up addressed by post-M122 planning: M123
...
- M168 follow-up: add direct `lower_generation_expression(...)` coverage for
  ...
  touched again.
- M170 resolved the M169 type-query module-size follow-up by extracting the
...
- M172 test follow-up: add a direct positive `base::signed_of(...)`
  vector-transform alias case if the concrete-vector extraction area is
  touched again. M172 covered the resolved unsigned-transform case.
- M172 evidence/docs follow-up: keep wording precise. Current
  selector-relevant member-alias evidence is specifically
  `vector::mask_underlying_t` under a `MaskVec` alias; transform-alias
  evidence is better represented by `StepVec`, `UVec`, `IndexVec`, and
  `SignedVec`.
- M172 follow-up feeding M173: vector-member aliases such as `MaskWord`,
  `MaskT`, and the `MaskVec` alias over
...
The final M179 validation run returned exit 0 for `git diff --check` with no
output, exit 0 for compileall with no output, exit 0 for the required pytest
command with `36 passed in 2.00s`, and exit 0 for the final cache check with
no output after validation-created `__pycache__` directories were removed.
The boundary auditor also ran a focused M164/M179 regression command and
reported `32 passed`; architecture, test, documentation, and validation
auditors reran relevant checks and reported clean results.

For post-M178 lowering planning, validation completed with:

```bash
git diff --check
```

The final post-M178 planning validation run returned exit 0 with no output.
...
The final M170 validation run returned exit 0 for `git diff --check` with no
output, exit 0 for compileall with no output, exit 0 for targeted pytest with
`382 passed in 25.92s`, and exit 0 for the final cache check with no output
after validation-created `__pycache__` directories were removed.
...
returned exit 0 with no output. The full clean-restart pytest command with
M163 tests returned exit 0 with `289 passed in 22.78s`. The first cache check
listed validation-created `__pycache__` directories under `tslgen/src/tslgen`
and `tslgen/tests`; they were removed, and the final
`find tslgen -type d -name __pycache__ -print` returned exit 0 with no
output.

For M162.5 execution and review, validation completed with:
...
`git diff --check` returned exit 0 with no output. The compileall command
returned exit 0 with no output. The full clean-restart pytest command with
M162.5 tests returned exit 0 with `272 passed in 26.73s`. The first cache
check listed validation-created `__pycache__` directories under
`tslgen/src/tslgen` and `tslgen/tests`; they were removed, and the final
`find tslgen -type d -name __pycache__ -print` returned exit 0 with no
output.
...
`tslgen/tests`; they were removed, and the final
`find tslgen -type d -name __pycache__ -print` returned exit 0 with no
output. A focused pre-review command also returned `21 passed, 235 deselected
in 11.01s`.

For M160 execution and review, validation completed with:
...
directories were removed, and the final
`find tslgen -type d -name __pycache__ -print` returned exit 0 with no
output. The validation auditor reported `Accept`.
...
For M111 clean binary operation lowering table slice, validation completed with:

```bash
git diff --check
python -B -m pytest -p no:cacheprovider tslgen/tests/test_m107_tiny_pipeline.py
python -B -c "from tslgen import Generator, Target, generate_from_paths"
python -B -c "from tslgen.lowering import BinaryOperationDescriptor, LoweredBinaryOperationExpression, lookup_binary_operation_descriptor"
python -B -m py_compile tslgen/src/tslgen/backends/cpp/backend.py tslgen/src/tslgen/backends/rust/backend.py tslgen/src/tslgen/domain/catalog.py tslgen/src/tslgen/lowering/__init__.py tslgen/src/tslgen/lowering/binary_operations.py tslgen/src/tslgen/lowering/lowerer.py tslgen/src/tslgen/lowering/model.py tslgen/src/tslgen/pipeline/catalog_builder.py tslgen/src/tslgen/syntax/parser.py tslgen/tests/test_m107_tiny_pipeline.py
```

`git diff --check` returned exit 0 with no output. The targeted clean-package
test command returned exit 0 with `24 passed`. The public API import command
returned exit 0 with no output. The binary operation lowering import command
...
For M113 clean function signature lowering slice, validation completed with:

```bash
git diff --check
python -B -m pytest -p no:cacheprovider tslgen/tests/test_m107_tiny_pipeline.py
python -B -c "from tslgen import Generator, Target, generate_from_paths"
python -B -c "from tslgen.lowering import LoweredFunctionSignature, LoweredFunction, Lowerer"
python -B -m py_compile tslgen/src/tslgen/backends/cpp/backend.py tslgen/src/tslgen/backends/rust/backend.py tslgen/src/tslgen/lowering/__init__.py tslgen/src/tslgen/lowering/lowerer.py tslgen/src/tslgen/lowering/model.py tslgen/tests/test_m107_tiny_pipeline.py
```

`git diff --check` returned exit 0 with no output. The targeted clean-package
test command returned exit 0 with `27 passed in 1.97s`. The public API import
...
For M115 clean binary division operation lowering slice, validation completed
with:
...
python -B -m pytest -p no:cacheprovider tslgen/tests/test_m107_tiny_pipeline.py
python -B -c "from tslgen import Generator, Target, generate_from_paths"
python -B -c "from tslgen.lowering import lookup_binary_operation_descriptor, supported_binary_operation_ids; assert supported_binary_operation_ids() == ('add', 'sub', 'mul', 'div'); assert lookup_binary_operation_descriptor('div') is not None"
python -B -m py_compile tslgen/src/tslgen/lowering/binary_operations.py tslgen/src/tslgen/backends/cpp/backend.py tslgen/src/tslgen/backends/rust/backend.py tslgen/tests/test_m107_tiny_pipeline.py
find tslgen -type d -name __pycache__ -print
```

`git diff --check` returned exit 0 with no output. The targeted clean-package
test command returned exit 0 with `36 passed in 2.23s`. The public API import
...
For M116 clean integer remainder operation type-gated lowering slice,
...
python -B -m pytest -p no:cacheprovider tslgen/tests/test_m107_tiny_pipeline.py
python -B -c "from tslgen import Generator, Target, generate_from_paths"
python -B -c "from tslgen.lowering import lookup_binary_operation_descriptor, supported_binary_operation_ids; assert supported_binary_operation_ids() == ('add', 'sub', 'mul', 'div', 'mod'); assert lookup_binary_operation_descriptor('mod') is not None"
python -B -m py_compile tslgen/src/tslgen/lowering/binary_operations.py tslgen/src/tslgen/lowering/operation_type_compatibility.py tslgen/src/tslgen/lowering/lowerer.py tslgen/src/tslgen/backends/cpp/backend.py tslgen/src/tslgen/backends/rust/backend.py tslgen/tests/test_m107_tiny_pipeline.py
find tslgen -type d -name __pycache__ -print
```
...
For M118 clean unary bitwise-not lowering shape slice, validation completed
with:
...
python -B -m pytest -p no:cacheprovider tslgen/tests/test_m107_tiny_pipeline.py
python -B -c "from tslgen import Generator, Target, generate_from_paths"
python -B -c "from tslgen.lowering import lookup_unary_operation_descriptor, supported_unary_operation_ids; assert supported_unary_operation_ids() == ('bit_not',); assert lookup_unary_operation_descriptor('bit_not') is not None"
python -B -m py_compile tslgen/src/tslgen/syntax/parser.py tslgen/src/tslgen/syntax/ast.py tslgen/src/tslgen/domain/catalog.py tslgen/src/tslgen/pipeline/catalog_builder.py tslgen/src/tslgen/lowering/unary_operations.py tslgen/src/tslgen/lowering/model.py tslgen/src/tslgen/lowering/lowerer.py tslgen/src/tslgen/lowering/operation_type_compatibility.py tslgen/src/tslgen/backends/cpp/backend.py tslgen/src/tslgen/backends/rust/backend.py tslgen/tests/test_m107_tiny_pipeline.py
find tslgen -type d -name __pycache__ -print
```

`git diff --check` returned exit 0 with no output. The targeted clean-package
test command returned exit 0 with `61 passed in 8.92s`. The public API import
command returned exit 0 with no output. The unary-operation descriptor import
command returned exit 0 with no output. The py_compile command returned exit 0
with no output. Validation-created `__pycache__` directories were removed, and
the final cache check returned exit 0 with no output.

For M119 clean unary arithmetic negation type-gated lowering slice,
validation completed with:
...
python -B -m pytest -p no:cacheprovider tslgen/tests/test_m107_tiny_pipeline.py
python -B -c "from tslgen import Generator, Target, generate_from_paths"
python -B -c "from tslgen.lowering import lookup_unary_operation_descriptor, supported_unary_operation_ids; assert supported_unary_operation_ids() == ('bit_not', 'neg'); assert lookup_unary_operation_descriptor('neg') is not None"
python -B -m py_compile tslgen/src/tslgen/lowering/unary_operations.py tslgen/src/tslgen/lowering/operation_type_compatibility.py tslgen/src/tslgen/backends/cpp/backend.py tslgen/src/tslgen/backends/rust/backend.py tslgen/tests/test_m107_tiny_pipeline.py
find tslgen -type d -name __pycache__ -print
```

`git diff --check` returned exit 0 with no output. The targeted clean-package
test command returned exit 0 with `69 passed`. The public API import command
returned exit 0 with no output. The unary-operation descriptor import command
returned exit 0 with no output. The py_compile command returned exit 0 with no
...
For M121 clean scalar compare result lowering shape slice, validation completed
with:
...
python -B -m pytest -p no:cacheprovider tslgen/tests/test_m107_tiny_pipeline.py
python -B -c "from tslgen import Generator, Target, generate_from_paths"
python -B -c "from tslgen.lowering import ComparisonOperationDescriptor, LoweredComparisonOperationExpression, SCALAR_COMPARISON_RESULT_TYPE, lookup_comparison_operation_descriptor, supported_comparison_operation_ids; assert supported_comparison_operation_ids() == ('equal',); assert lookup_comparison_operation_descriptor('equal') is not None"
python -B -m py_compile tslgen/src/tslgen/syntax/parser.py tslgen/src/tslgen/syntax/ast.py tslgen/src/tslgen/domain/catalog.py tslgen/src/tslgen/pipeline/catalog_builder.py tslgen/src/tslgen/lowering/comparison_operations.py tslgen/src/tslgen/lowering/model.py tslgen/src/tslgen/lowering/lowerer.py tslgen/src/tslgen/lowering/__init__.py tslgen/src/tslgen/backends/cpp/backend.py tslgen/src/tslgen/backends/rust/backend.py tslgen/tests/test_m107_tiny_pipeline.py
find tslgen -type d -name __pycache__ -print
```

`git diff --check` returned exit 0 with no output. The targeted clean-package
test command returned exit 0 with `91 passed in 16.29s`. The public API import
...
For post-M122 lowering planning, validation completed with:

```bash
git add -N docs/agent/runs/m123-execution-review-loop-prompt.md
git diff --check
```
