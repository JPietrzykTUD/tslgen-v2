...
```text
M162.5 execution-review returned Accept With Follow-Ups after one
write-capable executor and read-only architecture, boundary, evidence, test,
documentation, and validation audits.

M162.5 added `tslgen.syntax.tsil_lexical` as a small lexical-only helper for
balanced delimiter matching, top-level payload splitting, selector terminator
discovery, and raw brace-depth updates. It migrated accepted directive,
primitive-call, selector-payload, type-syntax, generation-loop, and
generation-control users where the helper directly fit while preserving
keyword-specific semantics, diagnostics, source locations, token identity, and
generated bytes.

M162.5 did not add TSIL keyword semantics, accepted source forms, renderer
behavior, expression parsing, source repair, broad parser frameworks,
registries, dispatchers, worklists, runtime `tsldata`, `frozen`, or
`tslgenold` dependencies. Token-level raw-brace close matching for loop and
generation-control regions intentionally remains local because those functions
own domain-specific diagnostics and branch/loop connector behavior.

M162.5 review verdicts were: architecture `Accept`; boundary `Accept`;
evidence `Accept With Follow-Up`; test `Accept`; documentation
`Accept With Follow-Ups`; validation `Accept With Follow-Up`. Documentation
follow-ups were addressed by refreshing the M163 prompt and the missing
lowering inventory. The remaining nonblocking follow-up is to keep future
token-region brace helper extraction explicit rather than folding
domain-specific diagnostics into the lexical helper.
```
...
should use the M162.5 shared lexical helper for top-level comma splitting and
raw brace-depth tracking where that directly fits. It must not render
declarations, evaluate initializer expressions, resolve symbols, infer types,
parse assignments/array access/calls/casts/intrinsics, or special-case
