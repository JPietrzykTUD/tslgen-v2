# Post-M70 Planning Plus Review Prompt
...
Milestones 1 through 70 are accepted. M70 returned
...
Do not implement code in this prompt, and do not start M71 execution.
...
Select exactly one next milestone candidate for post-M70 execution.
...
Compare lowering-focused candidates that could reasonably follow M70.
Potential directions include, but are not limited to:

- exact vector-alignment request resolution for
  `value<generation>(vector::alignment)`, consuming typed M67/M68/M69/M70
  request/result/pipeline values and explicit typed alignment metadata supplied
  before lowering evaluation;
- exact backend-uninit request modeling for
  `value<backend>(uninit::array)`, only if it remains typed lowering request
  modeling/resolution and does not produce backend text, translation requests,
  renderer-ready values, or generated output;
- a typed vector metadata input boundary if alignment resolution cannot be
  stated without speculative policy, but only if the boundary has concrete
  validation value and does not become a broad metadata registry;
- exact array-initialization declaration or array-type typed IR, only if it
  ...
  `array_type`, allocation, lifetime, store, return, or rendering semantics;
...
If a vector metadata or vector-alignment candidate is selected, it must explain
where vector facts come from as typed inputs before lowering evaluation. It
must not query host CPU features, read `tsldata`, read the catalog, parse raw
TSL, infer broad SVE/register semantics, or derive values from extension
spelling, vector-bit strings, selected type tags, scalar sizes, or
`candidate_id` parsing during lowering evaluation.

If a helper-resolution candidate is selected, require it to consume typed M67
request/provenance IR and the extracted M69/M70 stage pipeline boundary. It
must resolve exactly the selected request family. It must not reparse raw
...
accepted stage outputs such as M70 vector-length resolutions, M68 base-type
resolutions, M67 helper requests, M66 slot forms, or M65 envelope values and
to produce typed intermediate values. It must not produce renderer-ready text
or backend translation requests.
...
3. Boundary auditor: identify scope risks, especially vector metadata policy,
   helper evaluation, backend-uninit/backend/rendering work, raw helper
   dispatch, and broad TSIL parsing.
4. Evidence auditor: verify evidence paths and whether the candidate is
   supported by accepted M57-M70 behavior plus current corpus evidence.
5. Documentation auditor: check which roadmap/spec/testing docs need planning
   updates and whether stale M70 wording remains.
