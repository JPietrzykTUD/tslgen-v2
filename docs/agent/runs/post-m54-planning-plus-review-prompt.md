...
Produce a docs-only post-M54 planning update and internally review it with
subagents.
...
- Propose the next numbered milestone after M54, or recommend explicit
  deferral.
- Consider deferred parity targets after M54, including additional
  generation-time value/type lowering, backend modifier or type/value
  translation expansion, vector/register metadata, direct-intrinsic or
...
- Treat M49-M54 non-blocking follow-ups as cleanup candidates, not automatic
...
- Identify docs that must be updated for the post-M54 plan.
- Find stale post-M53/M54 wording.
...
- Verify the proposed next milestone preserves M40-M54 boundaries.
...
- Do not implement code.
- Do not start a Milestone 55 executor.
...
- Do not reopen accepted M40-M54 behavior unless evidence reveals a genuine
...
If the planning result requires human acceptance, create a post-M54 acceptance
