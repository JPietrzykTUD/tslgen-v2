# M190 Execution Review Loop Prompt

Execute this prompt only when `docs/agent/current-redesign-state.md` points
here and records M189 as accepted.
...
```text
M189: Typed Machine Feature Profile Catalog And Buildsystem Options Boundary
```
...
- M189 machine profile code under `tslgen/src/tslgen/domain/machine_profiles.py`
  and `tslgen/src/tslgen/pipeline/machine_profiles.py`
...
- Machine feature profile changes.
- Lowering changes.
- Dependency closure.
...
- Preserve the M189 machine profile boundary. Do not add compiler capability
  policy or host autodetection here.
...
1. Architecture/boundary auditor: verify typed backend metadata cataloging
   does not become backend semantic evaluation, renderer-side inference, or
   raw dictionary semantics.
...
- update `docs/redesign/implementation-roadmap.md` with the M190 result;
