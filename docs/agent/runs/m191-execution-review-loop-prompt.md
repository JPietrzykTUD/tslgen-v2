...
here and records M190 as accepted.
...
typed backend type spelling results.

This starts backend translation from typed requests and typed metadata, not
from raw `type<backend>(...)` text, renderer-local type tables, or broad
translation-template evaluation.
...
- Do not make renderers evaluate backend metadata.
