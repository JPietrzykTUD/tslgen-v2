# Post-M58 Planning + Review Prompt

You are the Codex orchestrator for the clean-room `tslgen` redesign.

Milestones 1 through 58 have been reviewed and accepted. The M58
execution-review loop returned `Accept With Follow-Ups` after one focused
documentation revision.

Your task is to plan the next numbered milestone after M58 and then run an
...
Produce a docs-only post-M58 planning update and internally review it with
subagents.

For this planning pass, focus on lowering. Select at most one next numbered
milestone. If evidence does not support a small next lowering slice, explicitly
defer implementation and record the stop or planning condition.
...
- Propose the next numbered milestone after M58, or recommend explicit
  deferral.
- Focus on lowering candidates that build on the accepted staged path:
  M55 typed size-byte values, M56 typed size-bit values, M57 typed size-byte
  equality predicates, and M58 typed stage records.
- Strong candidates to evaluate include the draft staged-lowering path already
  recorded in the roadmap:
  - size-byte equality branch-chain pruning over typed M57/M58 predicate
    outputs;
  - opaque selected branch body handoff after pruning.
- Consider whether any small M58 follow-up must precede branch-chain pruning,
  especially avoiding duplicate private staged-predicate assembly and ensuring
  future pruning consumes typed stage outputs rather than raw recognition text.
- Treat backend modifier/type-spelling expansion, direct-intrinsic/SVE body
  lowering, vector/register metadata, generated output, CLI/reporting, Rust,
  compiler execution, and M49-M58 cleanup follow-ups as deferred unless the
  evidence clearly supports one tiny lowering-only boundary.
...
- Verify evidence paths for plausible next lowering slices.
...
- Identify docs that must be updated for the post-M58 plan.
- Find stale post-M58 wording, including planning/execution wording around M58.
...
- Verify the proposed next milestone preserves M40-M58 boundaries.
- Confirm it does not combine generation-time lowering, backend translation,
  rendering, output, generated tests, CLI/reporting, Rust, and compiler work
...
- Confirm lowering rule/value/predicate/stage work does not make lowering read
  files, parse raw TSL, or query the catalog at evaluation time.
...
- Focus on milestone clarity, evidence, docs consistency, and whether the next
  lowering slice is small enough.
...
- Do not implement code.
- Do not start a Milestone 59 executor.
- Do not select a broad TSIL parser, broad backend renderer, broad generated
  test framework, whole-report parity, or default compiler-execution
  milestone.
- Do not change generated output.
- Do not reopen accepted M40-M58 behavior unless evidence reveals a genuine
...
Before final response, create the next concrete prompt under
`docs/agent/runs/` according to `docs/agent/next-run-prompt-protocol.md`,
unless the planning pass intentionally records an explicit stop condition.

If the planning result requires human acceptance, create a post-M58 acceptance
