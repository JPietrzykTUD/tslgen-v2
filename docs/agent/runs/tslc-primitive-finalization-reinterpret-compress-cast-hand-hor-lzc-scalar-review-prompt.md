...
`hand`, `hor`, and `lzc_scalar`. The `cast` slice also removes the same
call-type-args gap from `convert_down`.
...
- The regenerated coverage inventory reports all 89 primitives as `VERIFIED`,
  with `66594 / 67062` lowered slots. The only remaining skip taxonomy category
  is `pruned (closure)`.
