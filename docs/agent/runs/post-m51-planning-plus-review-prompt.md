# Post-M51 Planning + Review Prompt

You are the Codex orchestrator for the clean-room `tslgen` redesign.

Milestones 1 through 51 have been reviewed and accepted. M51 returned
`Accept With Follow-Ups` after one focused documentation revision.

Your task is to plan the next numbered milestone after M51 and then run an
...
Produce a docs-only post-M51 planning update and internally review it with
subagents.
...
- Propose the next numbered milestone after M51, or recommend explicit
  deferral.
- Consider deferred parity targets after M51, including additional
  generation-time lowering, backend modifier expansion, backend type/value
  request translation, direct-intrinsic or semantic TSIL helper slices, broader
  generated test-source parity, CLI workflow compatibility, broader
  coverage/report parity, Rust output, and executable generated-test/toolchain
  orchestration.
- Treat M49, M50, and M51 non-blocking follow-ups as cleanup candidates, not
  automatic next-milestone scope.
...
- Identify docs that must be updated for the post-M51 plan.
- Find stale post-M50/M51 wording.
...
- Verify the proposed next milestone preserves M40-M51 boundaries.
...
- Do not implement code.
- Do not start a Milestone 52 executor.
...
- Do not reopen accepted M40-M51 behavior unless evidence reveals a genuine
...
If the planning result requires human acceptance, create a post-M51 acceptance
