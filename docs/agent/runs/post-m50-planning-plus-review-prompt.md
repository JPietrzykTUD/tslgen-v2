# Post-M50 Planning + Review Prompt

You are the Codex orchestrator for the clean-room `tslgen` redesign.

Milestones 1 through 50 have been reviewed and accepted. M50 returned
`Accept With Follow-Ups` after one focused revision.

Your task is to plan the next numbered milestone after M50 and then run an
...
Produce a docs-only post-M50 planning update and internally review it with
subagents.
...
- Propose the next numbered milestone after M50, or recommend explicit
  deferral.
- Consider deferred parity targets after M50, including CLI workflow
  compatibility, broader coverage/report parity, backend modifier expansion,
  direct-intrinsic or semantic TSIL helper slices, broader generated
  test-source parity, Rust output, and executable generated-test/toolchain
  orchestration.
- Treat M49 and M50 non-blocking follow-ups as cleanup candidates, not automatic
...
- Identify docs that must be updated for the post-M50 plan.
- Find stale post-M49/M50 wording.
- Check roadmap/state alignment and whether accepted M50 follow-ups should be
...
- Verify the proposed next milestone preserves M40-M50 boundaries.
...
- Do not implement code.
- Do not start a Milestone 51 executor.
...
- Do not reopen accepted M40-M50 behavior unless evidence reveals a genuine
...
If the planning result requires human acceptance, create a post-M50 acceptance
