# TSLc Primitive Documentation Batch Prompt: Bitwise Ops
...
Current inventory after the arithmetic complex batch:

```text
140 primitive declarations total
24 declarations with detailed_description
24 declarations with semantics
```
...
Keep the authored text precise and source-owned. For binary bitwise operations,
describe lane-wise bit operations over each lane's bit pattern. For masked
variants, describe zeroing versus pass-through behavior explicitly. For
`binary_andnot`, be explicit about operand order. For `inv`, describe bitwise
complement of the lane bit pattern.
