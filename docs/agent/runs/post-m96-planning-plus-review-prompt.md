# Post-M96 Planning Plus Review Prompt

You are planning the next redesign milestone after accepted M96.

Milestones 1 through 96 are accepted. M96 accepted:
...
completion while respecting the accepted M96 boundary.
...
- M96 creates a private Stage 8 lowering completion manifest over accepted
  `LoweringOperationPackageIr` values and explicit unresolved dependency
  references.
- `lowering_completion_manifest` means accepted Stage 8 package/provenance
  assembly status only. It does not mean semantic body completion, backend
  readiness, renderer readiness, executable readiness, or generated-output
  readiness.
- M96 preserves accepted M86 mini-TSIL leaf-return facts, accepted M92
  exact-array backend-handoff facts, accepted M95 selected-body
  direct-intrinsic facts, and unresolved backend-handoff dependency references
  only through accepted package entries and object references.
...
- `boundary.py` remains exactly 1,300 lines and `_operation_package_sources.py`
  remains exactly 819 lines. The next lowering slice should extract before
  adding ownership to either pressure point.
