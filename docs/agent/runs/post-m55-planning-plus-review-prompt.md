# Post-M55 Planning + Review Prompt

You are the Codex orchestrator for the clean-room `tslgen` redesign.

Milestones 1 through 55 have been reviewed and accepted. The M55
execution-review loop returned `Accept With Follow-Ups` after one focused
revision for strict exact-query parsing.

Your task is to plan the next numbered milestone after M55 and then run an
...
Produce a docs-only post-M55 planning update and internally review it with
subagents.
...
- Propose the next numbered milestone after M55, or recommend explicit
  deferral.
- Consider deferred parity targets after M55, especially next lowering slices
  that build on typed `GenerationValue(kind="type.size_bytes")`: arithmetic or
  comparisons over generation values, branch pruning based on size values,
  vector/register metadata, backend modifier or type/value translation
  expansion, direct-intrinsic or semantic TSIL helper slices, broader generated
...
- Treat M49-M55 non-blocking follow-ups as cleanup candidates, not automatic
...
- Identify docs that must be updated for the post-M55 plan.
- Find stale post-M54/M55 wording.
...
- Verify the proposed next milestone preserves M40-M55 boundaries.
...
- Do not implement code.
- Do not start a Milestone 56 executor.
...
- Do not reopen accepted M40-M55 behavior unless evidence reveals a genuine
...
If the planning result requires human acceptance, create a post-M55 acceptance
