# Post-M49 Planning + Review Prompt

You are the Codex orchestrator for the clean-room `tslgen` redesign.

Milestones 1 through 49 have been reviewed and accepted. M49 returned
`Accept With Follow-Ups`.

Your task is to plan the next numbered milestone after M49 and then run an
...
Produce a docs-only post-M49 planning update and internally review it with
subagents.
...
- Propose the next numbered milestone after M49, or recommend explicit
  deferral.
- Consider deferred parity targets after M49, including reporting/coverage,
  CLI workflow compatibility, backend modifier expansion, broader generated
  test-source parity, and executable generated-test/toolchain orchestration.
...
- Identify docs that must be updated for the post-M49 plan.
- Find stale post-M48/M49 wording.
- Check OQ-032/OQ-036 consistency and roadmap/state alignment.
- Check whether any non-blocking M49 follow-ups should be recorded for later
  cleanup rather than included in the next milestone.

### 4. Boundary Auditor Subagent

Task:

- Verify the proposed next milestone preserves M40-M49 boundaries.
...
- Do not implement code.
- Do not start a Milestone 50 executor.
- Do not select a broad TSIL parser, broad backend renderer, broad generated
  test framework, or default compiler-execution milestone.
- Do not change generated output.
- Do not reopen accepted M40-M49 behavior unless evidence reveals a genuine
...
If the planning result requires human acceptance, create a post-M49 acceptance
