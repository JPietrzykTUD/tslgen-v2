# Post-M52 Planning + Review Prompt

You are the Codex orchestrator for the clean-room `tslgen` redesign.

Milestones 1 through 52 have been reviewed and accepted. M52 returned
`Accept With Follow-Ups` after a documentation wording cleanup.

Your task is to plan the next numbered milestone after M52 and then run an
...
Produce a docs-only post-M52 planning update and internally review it with
subagents.
...
- Propose the next numbered milestone after M52, or recommend explicit
  deferral.
- Consider deferred parity targets after M52, including additional
  generation-time value/type lowering, backend modifier or type/value
  translation expansion, vector/register metadata, direct-intrinsic or semantic
  TSIL helper slices, broader generated test-source parity, CLI workflow
  compatibility, broader coverage/report parity, Rust output, and executable
  generated-test/toolchain orchestration.
- Treat M49, M50, M51, and M52 non-blocking follow-ups as cleanup candidates,
  not automatic next-milestone scope.
...
- Identify docs that must be updated for the post-M52 plan.
- Find stale post-M51/M52 wording.
...
- Verify the proposed next milestone preserves M40-M52 boundaries.
...
- Do not implement code.
- Do not start a Milestone 53 executor.
...
- Do not reopen accepted M40-M52 behavior unless evidence reveals a genuine
...
If the planning result requires human acceptance, create a post-M52 acceptance
