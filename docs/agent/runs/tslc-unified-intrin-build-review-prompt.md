...
- Missing build fields use backend/extension defaults; explicit fields override
  those defaults.
- `suffix=` and `infix=` accept text values or typed generation values, with
  type values mapped through selected extension/backend suffix metadata.
- `prefix=` remains text-only; calls that want the selected extension's default
  prefix omit the field.
...
- explicit `prefix=...` rejects type values and only accepts text;
...
```bash
rg -n 'intrin_compose<|IntrinComposeLowerer|ComposeModifiers|value<backend>\\(intrin::suffix|value<backend>\\(intrin::prefix' tsldata/primitives tslc/src/tslc tslc/tests
```

Only the corpus guard test should mention `intrin_compose<` in current
production/test files. The obsolete backend-value intrinsic wrapper forms
should have no production/test hits.
