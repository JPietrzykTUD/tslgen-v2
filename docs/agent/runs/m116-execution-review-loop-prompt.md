# M116 Execution Review Loop Prompt

You are executing and reviewing the accepted next milestone after M115:

```text
Milestone 116: Tiny Clean Integer Remainder Operation Type-Gated Lowering Slice
```

Milestones 1 through 115 are accepted. M106 moved the pre-restart top-level
...
binary division through the existing backend-neutral descriptor path. Do not
start from old `tslgenold/` modules.

This milestone intentionally keeps the next task focused on lowering while
adding a small operation/type compatibility boundary.
...
Run exactly one write-capable executor for M116. The executor should:

1. Inspect dirty worktree state before editing and preserve unrelated changes.
2. Add `mod` to the existing lowering-owned binary operation descriptor table,
   preserving deterministic descriptor ordering.
3. Introduce only a small lowering-owned operation/type compatibility boundary
   over the existing M110 scalar descriptors and M111/M115 operation
   descriptors.
4. Accept `mod` only for currently supported integer scalar descriptors and
...
6. Update C++ and Rust backends to spell only the accepted `mod` descriptor via
   backend-owned operator spelling tables.
7. Preserve accepted `add`/`sub`/`mul`/`div` behavior, M110 scalar descriptors,
...
8. Add focused tests for descriptor lookup/order, integer lowerer acceptance,
   floating-type rejection with the new diagnostic, backend spelling
   ownership, generator output for at least one integer `mod` source, M114
   stage-output pass-through for `mod`, and the updated
   unsupported-operation diagnostic boundary.
...
2. Architecture reviewer: verify `mod` extends the existing typed binary
   operation descriptor path, the new type-gating remains a tiny lowering-owned
   compatibility boundary, and lowering does not gain backend spelling,
   runtime arithmetic policy, registries, dispatchers, or broad frameworks.
3. Documentation auditor: verify behavior, roadmap, and workflow state remain
   coherent and do not describe M116 as parser/source syntax, floating modulo,
...
If M116 review returns `Accept` or `Accept With Follow-Ups`:

- update `docs/agent/current-redesign-state.md`;
- mark M116 accepted in `docs/redesign/implementation-roadmap.md`;
- record follow-ups in state if any;
- create the next concrete run prompt under `docs/agent/runs/`.

Do not start Milestone 117 in this prompt.
