# Post-M81 Planning Plus Review Prompt
...
Milestones 1 through 81 are accepted. M81 returned
`Accept With Follow-Ups` after one focused maintainability revision.

Do not implement code in this prompt, and do not start Milestone 82 execution.
...
M81 preserved accepted M42-M80 behavior while:

- creating private generation-time lowering modules:
...
- moving accepted generation-time model, query, control-flow, and diagnostic
  helper ownership out of `boundary.py`;
- keeping source adapters, exact-array stage orchestration, and concrete
  `GenerationContext` construction that belongs to the facade in `boundary.py`;
...
- reducing `boundary.py` from the post-M80 7,208-line baseline to 5,438
  physical lines, below the M81 threshold of 5,808 lines;
- adding focused ownership/import-boundary tests for the new generation
  modules while preserving the full lowering-boundary suite.

M81 follow-ups are non-blocking:
...
- Continuing M80 maintainability follow-up: the selected/no-selected body
  envelope seam still uses narrow structural protocols/casts because concrete
  M63 envelope models remain facade-owned. A future lowering slice should
  either move selected-body envelope ownership deliberately or keep
  selected-body concrete checks at the facade boundary.
...
Select exactly one next milestone candidate for post-M81 execution.
...
Compare lowering-focused candidates that could reasonably follow M81.
Potential directions include, but are not limited to:

- a behavior-preserving selected-body envelope ownership extraction that
  resolves the selected/no-selected envelope seam without changing public
  imports or broadening protocols;
...
- a focused exact-array pipeline readability/typed-boundary cleanup that
  ...
  fixpoint engine;
- exact return-emission structural/request IR that consumes accepted M74-M81
...
M81's private `_generation_models.py`, `_generation_queries.py`,
`_generation_control_flow.py`, and `_generation_diagnostics.py` modules are
accepted lowering internals, not public API commitments or permission for
...
through M74-M81, but it must not interpret `emit_return`, `tmp`, return value
...
3. Boundary auditor: identify scope risks, especially M81 private-boundary
...
4. Evidence auditor: verify evidence paths and whether the candidate is
   supported by accepted M57-M81 behavior plus current corpus evidence.
5. Documentation auditor: check which roadmap/spec/testing docs need planning
   updates and whether stale M81 wording remains.
...
Do not create an M82 execution prompt until the selected post-M81 planning
