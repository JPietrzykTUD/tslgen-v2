# Post-M75 Planning Plus Review Prompt
...
Milestones 1 through 75 are accepted. M75 returned
...
extensibility, documentation, or evidence issues after that revision.

Do not implement code in this prompt, and do not start Milestone 76 execution.
...
Select exactly one next milestone candidate for post-M75 execution.
...
Compare lowering-focused candidates that could reasonably follow M75.
Potential directions include, but are not limited to:

- exact post-branch store-call structural/request refinement that consumes the
  accepted M75 predicate-path value and remains structural/provenance-only,
  without interpreting `svst1`, `tmp.data()`, `a`, alignment, memory behavior,
  store semantics, backend maps, rendering, or generated output;
- exact return-emission structural/request refinement that consumes accepted
  M74/M75 state and records only the exact `emit_return(tmp)` path without
...
- exact backend-uninit deferred-value handoff refinement that consumes the
  accepted M72/M73/M75 path and remains an unresolved typed backend-value
  boundary, without backend translation or renderer text;
- focused typed handoff from M75 predicate-path state to a later lowering
  boundary, only if it does not define variable scope, use-def analysis,
  allocation/lifetime, initializer behavior, statement execution semantics,
  store semantics, return semantics, or generic body IR;
- focused private maintainability cleanup of the repeated exact resolver,
  ...
  only if it is driven by concrete M68-M75 pressure and does not create a
  broad registry, central dispatcher, raw-helper evaluator, or public IR
  surface beyond genuinely consumed typed boundary values.
...
selected, it must consume accepted typed inputs such as M75 predicate-path IR,
M74 structural sequence IR, M73 declaration-shell IR, M72 helper-set
completions, M71 vector-alignment resolutions, M70 vector-length resolutions,
M68 base-type resolutions, M67 helper requests, M66 slot forms, M65 envelope
values, M64 slot-envelope values, or M63 selected-body envelope values. It
...
declaration semantics, generic array semantics, generic body semantics,
allocation/lifetime semantics, initializer semantics, variable scope
semantics, stores, returns, `tmp.data()`, `emit_return`, `assume_aligned`, or
broad `var` / `array_type` / call parsing.

If a store-call-shaped candidate is selected, it must stay structural/request
only. It may record exact tokens and provenance already carried through M75,
but it must not interpret `svst1`, `pg`, `tmp.data()`, `a`, memory behavior,
alignment behavior, backend intrinsic semantics, or renderer-ready output.
...
anchored to the exact accepted M68-M75 pattern. It must not add a broad stage
...
3. Boundary auditor: identify scope risks, especially M75-to-next-body-slot
...
4. Evidence auditor: verify evidence paths and whether the candidate is
   supported by accepted M57-M75 behavior plus current corpus evidence.
5. Documentation auditor: check which roadmap/spec/testing docs need planning
   updates and whether stale M75 wording remains.
