# Post-M57 Planning + Review Prompt

You are the Codex orchestrator for the clean-room `tslgen` redesign.

Milestones 1 through 57 have been reviewed and accepted. The M57
execution-review loop returned `Accept With Follow-Ups` with no blocking
implementation issues and no focused revision.

Your task is to plan the next numbered milestone after M57 and then run an
...
Produce a docs-only post-M57 planning update and internally review it with
subagents.
...
- Propose the next numbered milestone after M57, or recommend explicit
  deferral.
- Focus on lowering candidates that build on the accepted staged path:
  M55 typed size-byte values, M56 typed size-bit values, and M57 typed
  size-byte equality predicates.
- Strong candidates to evaluate include the draft staged-lowering path already
  recorded in the roadmap:
  - generation-time lowering stage pipeline boundary;
  - size-byte equality branch-chain pruning over typed M57 predicates;
...
- Treat backend modifier/type-spelling expansion, direct-intrinsic/SVE body
  ...
  compiler execution, and M49-M57 cleanup follow-ups as deferred unless the
...
- Identify docs that must be updated for the post-M57 plan.
- Find stale post-M57 wording, including planned/pending wording around M57
  predicate lowering.
...
- Verify the proposed next milestone preserves M40-M57 boundaries.
...
- Confirm lowering rule/value/predicate work does not make lowering read
...
- Do not implement code.
- Do not start a Milestone 58 executor.
...
- Do not reopen accepted M40-M57 behavior unless evidence reveals a genuine
  design inconsistency.
- Do not import or execute `frozen/`.
- Do not let review subagents implement changes directly; the orchestrator
  owns any final docs-only corrections.
...
If the planning result requires human acceptance, create a post-M57 acceptance
