...
- Plain `clang++ --target=aarch64-linux-gnu` still needs a clang-compatible
  aarch64 C++ sysroot/standard library in this environment, but C++ NEON
  runtime verification now works through Zig's bundled aarch64-musl toolchain
  using the existing verifier compiler/target knobs.
- SVE/scalable-vector emission remains deferred for a separate design pass.

### Native Register Metadata Guardrail

The NEON review follow-up is fixed. Fixed-width non-x86 native extensions now
must have declared backend register metadata before lowering can emit them.

Implementation notes:

1. `translation_common.requires_declared_vector_register(...)` identifies
   fixed-width non-x86 native substrates whose register types are source-owned
   extension metadata.
2. C++ and Rust backend type dialects return `None` for those substrates when
   `vector_register_types` lacks the selected backend/type spelling.
3. The existing lowerer diagnostic boundary reports
   `TSL-LOWER-NO-REGISTER-TYPE` for missing source-vector register spellings.
4. Representation-change targets now check the same boundary before creating
   `TargetVector`, so a target register cannot silently carry `None` into a
   renderer.
5. SVE remains deferred by the scalable-vector support-policy check, while
   NEON remains selected and rendered from typed register metadata.
...
### C++ NEON Runtime Verification Through QEMU

The C++ NEON runtime-verification follow-up is complete for the narrow `add`
slice and its dependency closure.

Environment findings:

1. `clang++ --target=aarch64-linux-gnu` is present but cannot compile a trivial
   C++/NEON probe in this image because standard library headers such as
   `<array>` and target libc headers are unavailable.
2. `/opt/zig/zig c++ -target aarch64-linux-musl` can compile and link a static
   aarch64 C++/NEON probe when Zig cache directories are placed under `/tmp`.
3. `/usr/bin/qemu-aarch64 -cpu cortex-a76` can run the resulting musl aarch64
   binary.

Implementation notes:

1. AArch64 NEON C++ profiles declare an empty profile-owned `cpp_flags` list.
   The target triple already selects baseline AArch64/ASIMD, and Zig's clang
   frontend rejects redundant `-march=armv8-a+simd` as an unsupported CPU name.
   Full profile-specific C++ compiler flags, such as future SVE `-march=...`
   spellings, belong to machine profile data rather than `cpp_project.py`.
2. The ISA-neutral C++ `detail::lane_bitmask_int` helper moved from
   `tsl_x86_traits.hpp` into `tsl_core.hpp`, so native ARM profile headers can
   name integral masks without including x86-only `<immintrin.h>` support.
3. The C++ verifier now runs a target-specific preflight for profiles with a
   `cpp_target`. Plain clang without an aarch64 sysroot skips only C++ NEON
   instead of failing the whole host-buildable C++ gate.
4. No new CLI or profile metadata plumbing was needed; the existing
   `--cpp-compiler`, `--cpp-target`, and `--qemu-aarch64` verifier options are
   sufficient.
...
Result: generated `878` C++ specializations, built the NEON-profile value-test
binary for `aarch64-linux-musl`, and CTest ran it through
`qemu-aarch64 -cpu cortex-a76`: `1/1 Test #1: values Passed` with
`build/test-verified 7 commands`.

```bash
TSLC_BACKENDS=cpp TSLC_OUTPUT_ROOT=/tmp/tslc-verify-cpp env -u CXX ./verify.sh
```

Result: passed for the broad C++ build gate. C++ NEON was skipped by target
preflight because plain clang cannot find the aarch64 C++ sysroot in this
...
The C++ renderer no longer contains AArch64 `-march=...` literals; it derives
ordinary x86 `-m...` flags from feature tokens and forwards profile-owned C++
flags. Validation:
...
Result: `51 passed` after compileall passed.
...
Result: passed all targeted validations: `242` non-build tests collected, `5`
value-test build/run checks run serially, and `53` generated-build tests
passed across the generated-build shards.
...
### Latest Active TSLc Handoff: SVE Cast Runtime-Length Coverage Checkpoint

The SVE conversion/extract coverage follow-up made a focused `cast`
checkpoint and kept the full SVE C++ runtime gate green. Rust SVE remains
unsupported and was not attempted.

Implemented:

1. Replaced the two remaining SVE `cast` bodies that used
   `value<generation>(generic::runtime_length(ToType))`.
2. The bodies now derive `OutVec` / `OutBase` through existing typed
   `let<type>` queries and use the runtime scalable lane count expression
   `svcntb() / sizeof(OutBase)`.
3. Kept the fix entirely in `tsldata/primitives/conversion/cast.tsl`; no
   renderer, lane-model, or compiler semantic changes were made.
...
Result: focused SVE `cast` coverage improved from `213/241` to `217/241`; the
`generic::runtime_length(ToType)` skip reason is gone.

```bash
PATH=/opt/zig:$PATH PYTHONPATH=tslc/src python -m tslc.cli --sources tsldata --machine-profiles supplementary/buildsystem/machine_profiles.json --backends cpp --profiles sve --primitives cast --output-root ./tslctmp/sve-cast-checkpoint --test --value-test-warnings --qemu-aarch64 /usr/bin/qemu-aarch64 --cpp-compiler "zig c++" --cpp-target aarch64-linux-musl
```

Result: generated `979` specializations; C++ CTest passed.
...
Result: full SVE coverage improved from `4138 emitted / 4469 attempted` to
`4142 emitted / 4469 attempted`; full SVE C++ qemu generated `4142`
specializations and passed CTest.
...
### Latest Active TSLc Handoff: SVE Convert-Down Runtime-Buffer Checkpoint

The SVE representation-change follow-up closed the emitted `convert_down`
runtime-length gaps. Rust SVE remains unsupported and was not attempted.

Implemented:

1. Added an SVE-specific `convert_down` fallback in
   `tsldata/primitives/conversion/repr_change.tsl`.
2. Avoided SVE2-only `svqxtn*` narrowing under the current `requires [sve]`
   profile. A local compile probe showed those ACLE intrinsics require SVE2 or
   SME with the installed Zig/ACLE headers.
3. Used an SVE1-valid runtime-buffer body: store the scalable source register
   with `svst1`, saturating-cast lanes into a runtime output buffer, then load
   the output window with `svld1` using the target base suffix.
4. Removed SVE from the fixed-lane generic fallback that still depends on
   `value<generation>(vector::length)`.
...
Result: focused SVE `convert_down` coverage emitted `65/65` slots. The old
`13` skips from unresolved `value<generation>(vector::length)` are gone.

```bash
PATH=/opt/zig:$PATH PYTHONPATH=tslc/src python -m tslc.cli --sources tsldata --machine-profiles supplementary/buildsystem/machine_profiles.json --backends cpp --profiles sve --primitives convert_down --output-root ./tslctmp/sve-convert-down-checkpoint --test --value-test-warnings --qemu-aarch64 /usr/bin/qemu-aarch64 --cpp-compiler "zig c++" --cpp-target aarch64-linux-musl
```

Result: focused SVE C++ qemu generated `827` specializations and passed CTest.
...
Result: full SVE C++ coverage improved to `4182 emitted / 4469 attempted`;
...
### Latest Active TSLc Handoff: SVE Cast/Reinterpret Checkpoint

The SVE conversion follow-up closed the remaining `cast` coverage gap. Rust
SVE remains unsupported and was not attempted.

Implemented:

1. Changed SVE `reinterpret` in
   `tsldata/primitives/conversion/cast.tsl` from marker-only target selector
   buckets to concrete arithmetic target groups. The body keeps identity
   reinterpretation as a generation-time `type::is_same(base::in, ToBase)`
   branch.
2. Fixed the SVE ACLE reinterpret spelling to the `svreinterpret_...` family.
3. Updated SVE integer widening casts to reinterpret the widened signed or
   unsigned temporary to the selected `ToBase`, so opposite-signed targets have
   the correct register type and static-cast bit behavior.
...
Result: focused SVE `cast` coverage emits `241/241` slots.

```bash
PATH=/opt/zig:$PATH PYTHONPATH=tslc/src python -m tslc.cli --sources tsldata --machine-profiles supplementary/buildsystem/machine_profiles.json --backends cpp --profiles sve --primitives cast --output-root ./tslctmp/sve-cast-checkpoint --test --value-test-warnings --qemu-aarch64 /usr/bin/qemu-aarch64 --cpp-compiler "zig c++" --cpp-target aarch64-linux-musl
```

Result: focused SVE C++ qemu generated `1039` specializations and passed
CTest.
...
Result: full SVE C++ coverage improved to `4337 emitted / 4495 attempted`;
...
Next action: close the SVE C++ `compress` / `compress_store`
...
### Latest Active TSLc Handoff: Inline Documentation Generation

Generated C++ and Rust APIs now include inline documentation from the primitive
documentation metadata plus lowered specialization facts.

Implemented:

1. Added `tslc.documentation` with typed `PrimitiveDocumentation` and
   `DocumentationBlock` values plus Doxygen/rustdoc formatting helpers.
2. Carried source-authored `brief_description`, `detailed_description`, and
   `semantics` through lowering on `LoweredSpecialization`.
3. Added C++ Doxygen comments for wrappers, specialization `apply(...)`
   methods, and public declarations. Generic wrapper docs use an `API` section
   for template parameters, return value, and runtime parameters. Concrete
   specialization docs use a `Specialization` section with readable labels such
   as `SIMD register` and concrete backend register spellings where available.
4. Added Rust rustdoc comments for dispatch traits, impl blocks, wrappers, and
   free functions using the same typed documentation boundary.
5. Added specialization tests proving C++ and Rust output include authored text,
   semantics code blocks, user-facing generic wrapper facts, concrete
   specialization facts, and no internal `Context`, `Result kind`, or
   `Parameter kinds` rows.
6. Recorded ADR-128.

Out of scope:

- No documentation site, CLI documentation mode, or separate artifact set.
- No interpretation of `semantics`; it remains raw documentation-only text.
- No catalog inspection or primitive-name classification inside renderers.
...
### Latest Active TSLc Handoff: React/Vite Specialization Inventory Explorer

The generated documentation site now includes a React/Vite specialization
explorer for the complete emitted specialization inventory.
...
1. Added `tslc.render.documentation_project`.
   ...
   - Includes primitive documentation, profile/backend/extension/family/type facts,
