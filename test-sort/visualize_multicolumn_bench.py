...
Design principle: **no hidden aggregation.** Every one of the six dimensions
(algorithm, distribution, data type, parallelism lanes, payload columns,
working-set size) is either placed on an axis (x / color / facet) or *pinned* to
a single value. Dimensions that are neither are impossible, so a plotted point is
always one measured configuration (median only over benchmark repetitions).
"""
...
DIMENSIONS = ["algo", "dtype", "dist", "lanes", "cols", "size"]
NUMERIC_X = ("lanes", "cols")
METRICS = {
    "ns_per_key": "ns / key element (lower is better)",
    "items_per_s": "keys / second (higher is better)",
...
BASELINE_KEYS = ["dtype", "dist", "cols", "size"]  # std_sort is lanes-independent
...
def parse_benchmarks(raw: dict) -> tuple[pd.DataFrame, dict]:
    ...
    for b in raw.get("benchmarks", []):
        ...
        rows.append(dict(
            algo=dims.get("algo", "?"), dtype=dims.get("type", "?"),
            lanes=int(b.get("lanes", 0)), dist=dims.get("dist", "?"),
            cols=int(b.get("cols", 0)), size=dims.get("size", "?"),
...
def add_speedup(df: pd.DataFrame) -> pd.DataFrame:
    for col in ("std_ns_per_key", "speedup_vs_std", "improvement_pct"):
        if df.empty:
            df[col] = pd.Series(dtype="float64")
    if df.empty:
        return df
    base = (df[df["algo"] == "std_sort"][BASELINE_KEYS + ["ns_per_key"]]
            .rename(columns={"ns_per_key": "std_ns_per_key"}).drop_duplicates(BASELINE_KEYS))
    df = df.merge(base, on=BASELINE_KEYS, how="left")
    df["speedup_vs_std"] = df["std_ns_per_key"] / df["ns_per_key"]
    df["improvement_pct"] = (1.0 - df["ns_per_key"] / df["std_ns_per_key"]) * 100.0
    return df
...
def dim_values(df: pd.DataFrame, dim: str) -> list:
    ...
    if dim == "lanes":
        vals = [v for v in vals if v > 0]  # drop the std_sort scalar sentinel
    order = {"size": SIZE_ORDER, "algo": ALGO_ORDER}.get(dim)
    return sorted(vals, key=lambda v: order.index(v) if order and v in order else v)


def default_pin(dim: str, vals: list):
    prefer = {"size": "LLC", "dtype": "u32", "dist": "uniform_random", "algo": "3way_net"}
...
def category_order(dim: str, values: list) -> list:
    ...
    if dim == "algo":
        return [a for a in ALGO_ORDER if a in strs]
    if dim in NUMERIC_X:
...
def build_chart(df: pd.DataFrame, x: str, color: str | None, facet_col: str | None,
    ...
    if metric in VS_STD:
        df = df[df["algo"] != "std_sort"]  # baseline is trivially 1x / 0%
    ...
    kwargs = dict(
        ...
        category_orders=orders, hover_data=axes, labels={metric: METRICS[metric]},
    )
...
PRESETS = {
    ...
    "Parallelism scaling": dict(x="lanes", color="algo", facet="dist", chart="line"),
...
def run_app() -> None:
    ...
    # apply pins; std_sort is lanes-independent so a lanes pin keeps it
    view_df = df
    for dim, value in pins.items():
        if dim == "lanes":
            view_df = view_df[(view_df["lanes"] == value) | (view_df["algo"] == "std_sort")]
        else:
    ...
    elif active_metric in VS_STD and view_df[view_df["algo"] != "std_sort"].empty:
        st.warning("Improvement metrics need a non-std_sort algorithm present for this slice.")
    else:
    ...
    with st.expander("Data coverage (values present per dimension)"):
        for dim in DIMENSIONS:
            ...
            order = dim_values(df, dim) if dim != "lanes" else sorted(df[dim].dropna().unique())
            st.write(f"**{dim}**: " + ", ".join(f"{v} ({int(counts.get(v, 0))})" for v in order))

    with st.expander(f"Filtered data ({len(view_df)} rows)"):
        table = view_df.sort_values(["dtype", "algo", "lanes", "dist", "cols", "size"])
