...
from tslc.authoring_metadata import (
    TextEdit,
    child_indent,
    entry_scope,
    first_field,
    ...
    list_text,
    replace_field_edit,
    safety_metadata_suggestions as compiler_safety_metadata_suggestions,
)
...
def safety_metadata_suggestions(
    ...
    *,
    ...
    """Adapt compiler-owned direct-safety edits to the audit report model."""
    ...
    return tuple(
        ...
        for suggestion in compiler_safety_metadata_suggestions(
            parsed,
            documents,
            path=path,
        )
...
def _requires_suggestions(
    ...
    *,
    ...
    entry_index = implementation_entry_index(inputs.parsed)
    ...
    for key, missing_features in sorted(
        ...
        suggestions.append(
            MetadataSuggestion(
                ...
                before=(
                    _render_requires_line(
                        child_indent(entry), sorted(local_flags)
                    ).rstrip()
                    if field is None
                    else field.source.text.rstrip()
                ),
                after=_render_requires_line(
                    child_indent(entry), sorted(after_flags)
                ).rstrip(),
                edit=edit,
...
def _requires_edit(
    ...
    rendered = _render_requires_line(child_indent(entry), sorted(flags))
    ...
    if entry.children:
        return None
    anchor = first_field(entry, "safety") or first_field(entry, "implementation")
    if anchor is None:
        return None
